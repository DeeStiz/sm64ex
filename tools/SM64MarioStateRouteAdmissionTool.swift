...
/// Admits exactly one source-backed Mario-state route shard after checking the
/// generated manifest, independent C/Swift schema-4 artifacts, and retained
/// Debug/ASan evidence. This tool intentionally does not construct records or
/// use `SM64RouteShardFixture`; the only records it can qualify are bytes read
/// from the two independent route traces.
@main
struct SM64MarioStateRouteAdmissionTool {
    private static let routeShardID: UInt64 = 0x88d0_4246_f94c_e9f8
    private static let routeInputSeed: UInt64 = 0x6b39_a105_2551_fd60
    private static let routeSaveSeed: UInt64 = 0x59a2_b6ab_37c5_b109
    private static let routeRecordFirst: UInt64 = 100
    private static let routeRecordLast: UInt64 = 118
    ...
    private enum ToolError: Error, CustomStringConvertible {
        ...
        var description: String {
            ...
            case let .wrongManifestRow(reason): return "Mario-state manifest row mismatch: \(reason)"
            ...
            case let .wrongRecordCount(label, count): return "\(label) record count \(count) is not \(routeRecordCount)"
            ...
            case .recordBytesMismatch: return "independent C/Swift trace record bytes differ"
    ...
    private struct RawTrace {
        ...
        var recordBytes: Data {
            Data(bytes.dropFirst(traceHeaderSize))
        }
    }
    ...
    static func main() {
        do {
            ...
            FileHandle.standardError.write(Data(("sm64-mario-state-route-admit: \(error)\n").utf8))
            exit(2)
        }
    }
    ...
    private static func run(arguments: [String]) throws {
        ...
        guard shard.inputSeed == routeInputSeed, shard.saveSeed == routeSaveSeed else {
            throw ToolError.wrongManifestRow("input/save seed does not match the route contract")
        }
        guard shard.expectedDomains == [.marioState] else {
            throw ToolError.wrongManifestRow(
                "expected domain set is \(shard.expectedDomains.map(\.rawValue))"
            )
        }
        ...
        try requireMarker(options.debugLog, "mario_state_route_init status=0 oracle=0")
        try requireMarker(options.debugLog, "mario_state_route_step index=0 status=0 oracle=0 parity=0")
        try requireMarker(options.debugLog, "mario_state_route_step index=1 status=0 oracle=0 parity=0")
        try requireMarker(options.swiftLog, "mario_state_pairing_audit admitted=1")
        try requireMarker(options.swiftLog, "first_divergence=none")
        try requireMarker(options.swiftLog, "mario_state_pairing_tamper_rejected=1")
        try requireMarker(options.asanLog, "asan_mario_state_route_passed=1")
        ...
        guard cTrace.bytes == swiftTrace.bytes else { throw ToolError.recordBytesMismatch }
        guard cTrace.bytes == asanTrace.bytes else { throw ToolError.recordBytesMismatch }
        ...
        guard afterPlanned == beforePlanned - 1, afterTerminal == beforeTerminal + 1 else {
            throw ToolError.ledgerMutation(
                ToolError.reportNotPristine(afterPlanned, afterTerminal)
            )
        }
        do {
        ...
        guard try ledger.state(for: routeShardID) == .passed else {
            throw ToolError.ledgerMutation(ToolError.reportAlreadyAdvanced(routeShardID, .running))
        }

        print(
            String(
                format: "SM64 Mario-state route admission passed shard=0x%016llx "
                    + "records=%d ticks=2,3 before_planned=%d before_terminal=%d "
                    + "after_planned=%d after_terminal=%d report=%@ "
                    + "c_trace=%@ swift_trace=%@ asan_trace=%@ tamper_rejected=1",
                routeShardID,
    ...
    private static func parse(arguments: [String]) throws -> Options {
        let usage = "usage: sm64-mario-state-route-admit --manifest MANIFEST --c-trace C_TRACE --swift-trace SWIFT_TRACE --tampered-trace TAMPERED_TRACE --debug-log DEBUG_LOG --swift-log SWIFT_LOG --asan-trace ASAN_TRACE --asan-log ASAN_LOG --report REPORT"
    ...
    private static func resolveShard(_ manifest: String) throws -> SM64RouteShard {
            ...
            do {
                ...
                if shard.id == routeShardID {
                    guard shard.domain == "oracle_hook",
                          shard.identity == "mario_state",
                          shard.source == "src/pc/sm64_modern_gameplay_parity.c",
                          shard.manifestState == .planned else {
                        throw ToolError.wrongManifestRow("identity/source/status does not match canonical route")
                    }
                    return shard
                }
        ...
        throw ToolError.invalidManifest(
            String(format: "missing shard 0x%016llx", routeShardID)
        )
    }
    ...
    private static func validateHeaders(
        ...
        guard c.regionCode == swift.regionCode else { throw ToolError.headerMismatch("region_code") }
        guard c.mode == swift.mode else { throw ToolError.headerMismatch("mode") }
        let cFingerprints = Fingerprints(c)
        let swiftFingerprints = Fingerprints(swift)
        let names = CanonicalFingerprintNames()
        let pairs: [(String, UInt64, UInt64)] = [
            (names.build, cFingerprints.build, swiftFingerprints.build),
            (names.content, cFingerprints.content, swiftFingerprints.content),
            (names.timebase, cFingerprints.timebase, swiftFingerprints.timebase),
            (names.configuration, cFingerprints.configuration, swiftFingerprints.configuration),
            (names.initialSave, cFingerprints.initialSave, swiftFingerprints.initialSave),
            (names.coverage, cFingerprints.coverage, swiftFingerprints.coverage),
        ]
        for (name, expected, actual) in pairs where expected != actual {
            throw ToolError.fingerprintMismatch(name, expected, actual)
        }
    }
    ...
    private static func validateCanonicalHeader(
        ...
        let expected = Fingerprints(
            SM64OracleTraceConfiguration(
                ...
                contentFingerprint: hashString("src/pc/sm64_modern_gameplay_parity.c|oracle_hook|mario_state"),
                timebaseFingerprint: timebaseFingerprint(),
                configurationFingerprint: hashString(
                    "region=5553;fullscreen=off;skip_intro=1;native_tick=1;legacy_tick=1;shard=0x88d04246f94ce9f8"
                ),
                initialSaveFingerprint: hashString("save=empty-us-slot-0;seed=0x59a2b6ab37c5b109"),
    ...
    private static func validateRecords(_ trace: RawTrace, label: String) throws {
        ...
        for (index, record) in trace.records.enumerated() {
            ...
            guard record.domain == SM64RouteShardTraceDomain.marioState.cDomain else {
                throw ToolError.wrongRecord(label, index, "domain \(record.domain) != 2")
            }
            guard record.recordKind == SM64RouteShardTraceDomain.marioState.cRecordKind else {
            ...
            guard (1...3).contains(record.values.count) else {
                throw ToolError.wrongRecord(label, index, "value count \(record.values.count) is not canonical")
            }
