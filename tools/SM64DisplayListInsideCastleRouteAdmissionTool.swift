...
/// isolated report.  The route manifest and all pair artifacts are immutable
...
/// gates.
@main
struct SM64DisplayListInsideCastleRouteAdmissionTool {
    ...
    private static let routeShardID: UInt64 = 0x009e_4310_51db_a428
    private static let routeInputSeed: UInt64 = 0x14ec_ced3_11bc_c690
    private static let routeSaveSeed: UInt64 = 0x2c48_3fa0_516b_9079
    private static let routeEvent: UInt64 = 0xd4
    ...
    private static let sourceIdentity: UInt64 = 0x3362_be68_84c9_7a75
    private static let ownerIdentity: UInt64 = 0x12f6_74c2_8303_04c8
    private static let routeLayer: UInt32 = 1
    private static let routeWordCount: UInt32 = 6
    private static let routeTriangleCount: UInt32 = 2
    private static let routeFlags: UInt32 = 3
    private static let routeResourceID: UInt32 = 0x0900_c800
    private static let routePacketFingerprint: UInt64 = 0x7d3e_94d7_a311_a393
    private static let routeWords: [UInt32] = [
        0xfd10_0000, 0x0900_c800,
        0xe600_0000, 0x0000_0000,
        0xf300_0000, 0x077f_f080,
        0x0100_4008, 0x0704_36c8,
        0x0600_0204, 0x0000_0602,
        0xdf00_0000, 0x0000_0000,
    ]
    private static let routeResources: [UInt32] = [0x0900_c800, 0, 0, 0x0704_36c8, 0, 0]
    private static let routeResourceFingerprint: UInt64 = 0x151c_59c2_9bb8_1547
    ...
    private enum ToolError: Error, CustomStringConvertible {
        ...
        var description: String {
            ...
            case let .wrongManifestRow(reason): return "inside-castle display-list manifest row mismatch: \(reason)"
            ...
            case let .headerMismatch(field): return "C/Swift/ASan/Release/rerun inside-castle header mismatch: \(field)"
            ...
            case let .invalidPacket(url, reason): return "invalid inside-castle display-list packet \(url.path): \(reason)"
    ...
    static func main() {
        do {
            ...
            FileHandle.standardError.write(Data(("sm64-display-list-inside-castle-route-admit: \(error)\n").utf8))
            exit(2)
        }
    }
    ...
    private static func run(arguments: [String]) throws {
        ...
        print(
            "SM64 inside-castle display-list route isolated admission passed "
                + "shard=\(formatID(routeShardID)) identity=inside_castle_seg7_dl_07043A68 "
                + "source=levels/castle_inside/areas/2/3/model.inc.c records=2 ticks=1,2 domain=11 kind=7 schema=4 "
    ...
    private static func parse(arguments: [String]) throws -> Options {
        let usage = "usage: sm64-display-list-inside-castle-route-admit --manifest MANIFEST --c-trace C_TRACE --swift-trace SWIFT_TRACE --asan-trace ASAN_TRACE --release-trace RELEASE_TRACE --rerun-trace RERUN_TRACE --tampered-trace TAMPERED_TRACE --c-packet C_PACKET --asan-packet ASAN_PACKET --release-packet RELEASE_PACKET --rerun-packet RERUN_PACKET --debug-log DEBUG_LOG --swift-log SWIFT_LOG --asan-log ASAN_LOG --release-log RELEASE_LOG --rerun-log RERUN_LOG --pair-log PAIR_LOG --report REPORT"
        guard arguments.count == 36, arguments.count.isMultiple(of: 2) else {
        ...
        return Options(
            ...
            pairLog: url("--pair-log"), report: url("--report")
        )
    }
    ...
    private static func requireDistinctEvidence(_ options: Options) throws {
        ...
        let outputs = [options.report, options.debugLog, options.swiftLog, options.asanLog, options.releaseLog, options.rerunLog, options.pairLog]
    ...
    private static func resolveTarget(_ rows: [ManifestRow]) throws -> ManifestRow {
        ...
        guard shard.domain == "display_list",
              shard.identity == "inside_castle_seg7_dl_07043A68",
              shard.source == "levels/castle_inside/areas/2/3/model.inc.c",
    ...
    private static func validateCanonicalHeader(_ configuration: SM64OracleTraceConfiguration, label: String) throws {
        let expected = SM64OracleTraceConfiguration(
            regionCode: 0x5553,
            mode: .record,
            buildFingerprint: hashString("sm64-modern-display-list-inside-castle-route-build-v1"),
            contentFingerprint: hashString("levels/castle_inside/areas/2/3/model.inc.c|inside_castle_seg7_dl_07043A68|render_packet"),
            ...
            configurationFingerprint: hashString(
                "region=5553;fullscreen=off;skip_intro=1;"
                    + "shard=0x009e431051dba428;parent=inside_castle_seg7_dl_07043B48;layer=1"
            ),
            initialSaveFingerprint: hashString("save=empty-us-slot-0;seed=0x2c483fa0516b9079"),
    ...
    private static func requireMarkers(_ options: Options) throws {
        try requireMarker(options.pairLog, "fixture_only=0")
        try requireMarker(options.pairLog, "manifest_reconciliation=authored_route_only")
        for marker in [
            "c_display_list_inside_castle_route_recorded shard=0x009e431051dba428 records=2 ticks=1,2",
            "display_list_inside_castle_route_debug oracle_end=0 result_status=0 records=2 invocations=2 matches=2 failures=0",
            "display_list_inside_castle_route_owner_fence=1 pointer_free_packet=1",
        ] { try requireMarker(options.debugLog, marker) }
        for marker in [
            "swift_display_list_inside_castle_route_recorded shard=0x9e431051dba428 records=2 ticks=1,2 words=6 triangles=2",
            "display_list_inside_castle_pairing_audit admitted=1 c_records=2 swift_records=2 blockers= first_divergence=none",
            "display_list_inside_castle_pairing_tamper_rejected=1",
