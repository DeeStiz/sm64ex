...
/*
 * C owner for the generated oracle_hook|collision_queries row.  The native
 * surface lists, partitions, and query functions remain authoritative.  This
 * harness only retains the fixed-width schema-4 receipts emitted by those
 * functions; it does not construct a surface, query, or route record.
 */
#define COLLISION_ROUTE_SHARD_ID UINT64_C(0xc3294578ae77bee8)
#define COLLISION_ROUTE_INPUT_SEED UINT64_C(0x90d1525c63a84e50)
#define COLLISION_ROUTE_SAVE_SEED UINT64_C(0xce793ba490fd0739)
#define COLLISION_ROUTE_TRACE_STEPS 2u
#define COLLISION_ROUTE_EVENT_FIRST SM64_MODERN_ORACLE_COLLISION_EVENT_FLOOR
#define COLLISION_ROUTE_EVENT_LAST SM64_MODERN_ORACLE_COLLISION_EVENT_ENVIRONMENT
#define COLLISION_ROUTE_EVENT_COUNT \
    (COLLISION_ROUTE_EVENT_LAST - COLLISION_ROUTE_EVENT_FIRST + 1u)
#define COLLISION_ROUTE_FNV_OFFSET UINT64_C(1469598103934665603)
#define COLLISION_ROUTE_FNV_PRIME UINT64_C(1099511628211)
...
struct TraceFile {
    ...
    bool have_record;
    bool retained_ids[COLLISION_ROUTE_EVENT_COUNT];
    uint32_t records_by_event[COLLISION_ROUTE_EVENT_COUNT];
    uint32_t tick_count;
    uint32_t failures;
};
...
static uint64_t hash_u64(uint64_t hash, uint64_t value) {
    for (uint32_t byte = 0; byte < 8u; ++byte) {
        ...
        hash *= COLLISION_ROUTE_FNV_PRIME;
    }
    return hash;
}

static uint64_t hash_string(const char *value) {
    uint64_t hash = COLLISION_ROUTE_FNV_OFFSET;
...
static uint64_t retained_coverage_fingerprint(const struct TraceFile *trace) {
    uint64_t hash = COLLISION_ROUTE_FNV_OFFSET;
    uint32_t count = 0;
    for (uint32_t index = 0; index < COLLISION_ROUTE_EVENT_COUNT; ++index) {
        if (!trace->retained_ids[index]) {
            continue;
        }
        hash = hash_u64(hash, SM64_MODERN_ORACLE_DOMAIN_COLLISION);
        hash = hash_u64(hash, 0u);
        hash = hash_u64(hash, COLLISION_ROUTE_EVENT_FIRST + index);
        count++;
    }
...
static uint32_t expected_value_count(uint64_t event_id) {
    switch (event_id) {
        case SM64_MODERN_ORACLE_COLLISION_EVENT_FLOOR:
        case SM64_MODERN_ORACLE_COLLISION_EVENT_CEIL:
            return 7u;
        case SM64_MODERN_ORACLE_COLLISION_EVENT_WALL:
            return 8u;
        case SM64_MODERN_ORACLE_COLLISION_EVENT_ENVIRONMENT:
            return 4u;
        default:
            return 0u;
    }
...
static SM64ModernStatus trace_write_record(
    ...
    /* Initialization, Mario, camera, object, and render records are outside
     * this generated collision row. */
    if (record->domain != SM64_MODERN_ORACLE_DOMAIN_COLLISION
        || record->record_kind != SM64_MODERN_ORACLE_RECORD_EVENT) {
        return SM64_MODERN_STATUS_OK;
    }
    if (record->record_id < COLLISION_ROUTE_EVENT_FIRST
        || record->record_id > COLLISION_ROUTE_EVENT_LAST
...
static SM64ModernLifecycleConfigV1 make_lifecycle_config(const char *save_directory) {
    ...
    snprintf(config.config_file, sizeof(config.config_file), "%s", "sm64-modern-collision-queries-route.cfg");
    snprintf(config.window_title, sizeof(config.window_title), "%s", "SM64 Modern Collision Queries Route");
    return config;
}
...
static SM64ModernOracleTraceConfigV1 make_oracle_config(uint64_t timebase_fingerprint) {
    ...
    config.mode = SM64_MODERN_ORACLE_TRACE_RECORD;
    config.build_fingerprint = hash_string("sm64-modern-collision-queries-route-build-v1");
    config.content_fingerprint = hash_string(
        "src/pc/sm64_modern_gameplay_parity.c|oracle_hook|collision_queries");
    ...
    config.configuration_fingerprint = hash_string(
        ...
        "shard=0xc3294578ae77bee8");
    config.initial_save_fingerprint = hash_string(
        "save=empty-us-slot-0;seed=0xce793ba490fd0739");
    return config;
}
...
static bool record_route(const char *trace_path, const char *save_directory) {
    ...
    if (ok) {
        ...
        fprintf(stderr, "collision_queries_route_init status=%u oracle=%u parity=%u\n",
        ...
        for (uint32_t index = 0; ok && index < COLLISION_ROUTE_TRACE_STEPS; ++index) {
            const SM64ModernStatus status = lifecycle.step();
            fprintf(stderr,
                    "collision_queries_route_step index=%u status=%u oracle=%u parity=%u\n",
    ...
    fprintf(stderr,
            "collision_queries_route_debug oracle_end=%u result_status=%u actual=%" PRIu64
            " retained=%" PRIu64 " failures=%u ticks=%u event_kinds=%u\n",
    ...
    if (!ok) {
        fprintf(stderr,
                "collision_queries_route_failed errors=%u first_error_status=%u message=%s\n",
                state.errors, state.first_error_status,
                state.first_error_message[0] ? state.first_error_message : "(none)");
        return false;
    }
    printf("c_collision_queries_route_recorded shard=0x%016" PRIx64
           ...
           ",save:0x%016" PRIx64 "\n",
           COLLISION_ROUTE_SHARD_ID, trace_path, trace.records, trace.last_tick,
           trace.records_by_event[0], trace.records_by_event[1],
           trace.records_by_event[2], trace.records_by_event[3],
           retained_coverage_fingerprint(&trace), config.build_fingerprint,
...
int main(int argc, char **argv) {
    if (argc != 3) {
        fprintf(stderr,
                "usage: sm64-modern-collision-queries-route-contract TRACE SAVE_DIRECTORY\n");
        return 2;
    }
