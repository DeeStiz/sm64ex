...
static uint64_t hash_tick(
    ...
    hash = hash_u64(hash, second_tick ? 12 : 14); // object counter
    hash = hash_u64(hash, second_tick ? 12 : 14); // dispatch events
    ...
    if (second_tick) {
        hash = hash_event(hash, 13, CHILD_BEHAVIOR, 255);
    } else {
        hash = hash_event(hash, 1, RESPAWNER_BEHAVIOR, 1);
        hash = hash_event(hash, 13, CHILD_BEHAVIOR, 255);
        hash = hash_event(hash, 12, BOBOMB_SMOKE_BEHAVIOR, 255);
    }
