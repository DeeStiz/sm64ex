...
static uint64_t hash_tick(
    ...
    hash = hash_u64(hash, second_tick ? 11 : 13); // object counter
    hash = hash_u64(hash, second_tick ? 11 : 13); // dispatch events
    ...
    if (second_tick) {
        hash = hash_event(hash, 12, CHILD_BEHAVIOR, 255);
    } else {
        hash = hash_event(hash, 1, RESPAWNER_BEHAVIOR, 1);
        hash = hash_event(hash, 12, CHILD_BEHAVIOR, 255);
        hash = hash_event(hash, 11, BOBOMB_SMOKE_BEHAVIOR, 255);
    }
