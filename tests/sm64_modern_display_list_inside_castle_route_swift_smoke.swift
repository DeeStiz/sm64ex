...
private let routeShardID: UInt64 = 0x009e_4310_51db_a428
private let routeDomain: UInt32 = 11
private let routeRecordKind: UInt32 = 7
private let routeEvent: UInt64 = 0xd4
...
private func readPacket(_ url: URL) throws -> RoutePacket {
    ...
    let expectedResources: [UInt32] = [0x0900_c800, 0, 0, 0x0704_36c8, 0, 0]
    guard source == 0x3362_be68_84c9_7a75,
          owner == 0x12f6_74c2_8303_04c8,
          wordCount == 6,
          layer == 1,
          triangles == 2,
          flags == 3,
...
private func writeRoute(cTrace: URL, packetURL: URL, swiftTrace: URL) throws {
    ...
    print(
        "swift_display_list_inside_castle_route_recorded shard=0x\(String(routeShardID, radix: 16))"
            + " records=\(route.records.count) ticks=1,2 words=6 triangles=2"
...
private func audit(cTrace: URL, swiftTrace: URL) throws {
    ...
    print(
        "display_list_inside_castle_pairing_audit admitted=1 c_records=\(c.records.count)"
...
private func tamper(input: URL, output: URL) throws {
    ...
    do {
        ...
        print("display_list_inside_castle_pairing_tamper_rejected=1")
    }
}

@main
struct SM64ModernDisplayListInsideCastleRouteSmoke {
    static func main() {
        do {
            ...
            fputs("display_list_inside_castle_route_swift_error=\(error)\n", stderr)
            exit(1)
        }
    }
}
