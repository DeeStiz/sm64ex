...
/*
 * This harness owns the C side of the generated oracle_hook|script_events
 * row.  Level/script callbacks remain the native authority; this stream only
 * retains their fixed-width schema-4 receipts.  No event is synthesized from
 * an opcode list or from a Swift fixture.
 */
#define SCRIPT_EVENTS_ROUTE_SHARD_ID UINT64_C(0x2b0f6063b5463e9c)
#define SCRIPT_EVENTS_ROUTE_INPUT_SEED UINT64_C(0xb3494e3c2c19d924)
#define SCRIPT_EVENTS_ROUTE_SAVE_SEED UINT64_C(0xcccad169f65f541d)
#define SCRIPT_EVENTS_ROUTE_TRACE_STEPS 2u
#define SCRIPT_EVENTS_ROUTE_EVENT_FIRST SM64_MODERN_ORACLE_SCRIPT_EVENT_LEVEL_COMMAND
#define SCRIPT_EVENTS_ROUTE_EVENT_LAST SM64_MODERN_ORACLE_SCRIPT_EVENT_LIFECYCLE
#define SCRIPT_EVENTS_ROUTE_EVENT_COUNT \
    (SCRIPT_EVENTS_ROUTE_EVENT_LAST - SCRIPT_EVENTS_ROUTE_EVENT_FIRST + 1u)
#define SCRIPT_EVENTS_ROUTE_FNV_OFFSET UINT64_C(1469598103934665603)
#define SCRIPT_EVENTS_ROUTE_FNV_PRIME UINT64_C(1099511628211)
...
struct TraceFile {
    ...
    bool have_record;
    bool retained_ids[SCRIPT_EVENTS_ROUTE_EVENT_COUNT];
    uint32_t records_by_event[SCRIPT_EVENTS_ROUTE_EVENT_COUNT];
    uint32_t tick_count;
    uint32_t failures;
};
...
static uint64_t hash_u64(uint64_t hash, uint64_t value) {
    for (uint32_t byte = 0; byte < 8u; ++byte) {
        ...
        hash *= SCRIPT_EVENTS_ROUTE_FNV_PRIME;
    }
    return hash;
}

static uint64_t hash_string(const char *value) {
    uint64_t hash = SCRIPT_EVENTS_ROUTE_FNV_OFFSET;
...
static uint64_t retained_coverage_fingerprint(const struct TraceFile *trace) {
    uint64_t hash = SCRIPT_EVENTS_ROUTE_FNV_OFFSET;
    uint32_t count = 0;
    for (uint32_t index = 0; index < SCRIPT_EVENTS_ROUTE_EVENT_COUNT; ++index) {
        if (!trace->retained_ids[index]) continue;
        hash = hash_u64(hash, SM64_MODERN_ORACLE_DOMAIN_SCRIPT);
        hash = hash_u64(hash, 0u);
        hash = hash_u64(hash, SCRIPT_EVENTS_ROUTE_EVENT_FIRST + index);
        count++;
    }
...
static SM64ModernStatus trace_write_record(
    ...
    /* Initialization/render/gameplay records are real but outside this row. */
    if (record->domain != SM64_MODERN_ORACLE_DOMAIN_SCRIPT
        || record->record_kind != SM64_MODERN_ORACLE_RECORD_EVENT) {
        return SM64_MODERN_STATUS_OK;
    }
    if (record->record_id < SCRIPT_EVENTS_ROUTE_EVENT_FIRST
        || record->record_id > SCRIPT_EVENTS_ROUTE_EVENT_LAST
    ...
    const uint32_t index = (uint32_t) record->record_id
        - SCRIPT_EVENTS_ROUTE_EVENT_FIRST;
...
static SM64ModernLifecycleConfigV1 make_lifecycle_config(const char *save_directory) {
    ...
    snprintf(config.config_file, sizeof(config.config_file), "%s", "sm64-modern-script-events-route.cfg");
    snprintf(config.window_title, sizeof(config.window_title), "%s", "SM64 Modern Script Events Route");
    return config;
}
...
static SM64ModernOracleTraceConfigV1 make_oracle_config(uint64_t timebase_fingerprint) {
    ...
    config.mode = SM64_MODERN_ORACLE_TRACE_RECORD;
    config.build_fingerprint = hash_string("sm64-modern-script-events-route-build-v1");
    config.content_fingerprint = hash_string(
        "src/pc/sm64_modern_gameplay_parity.c|oracle_hook|script_events");
    ...
    config.configuration_fingerprint = hash_string(
        ...
        "shard=0x2b0f6063b5463e9c");
    config.initial_save_fingerprint = hash_string(
        "save=empty-us-slot-0;seed=0xcccad169f65f541d");
    return config;
}
...
static bool record_route(const char *trace_path, const char *save_directory) {
    ...
    if (ok) {
        ...
        fprintf(stderr, "script_events_route_init status=%u oracle=%u parity=%u\n",
        ...
        for (uint32_t index = 0; ok && index < SCRIPT_EVENTS_ROUTE_TRACE_STEPS; ++index) {
            const SM64ModernStatus status = lifecycle.step();
            fprintf(stderr, "script_events_route_step index=%u status=%u oracle=%u parity=%u\n",
    ...
    fprintf(stderr, "script_events_route_debug oracle_end=%u result_status=%u actual=%" PRIu64
    ...
    /* The selected route need not cross every authored event kind (for
     * example, a two-tick Castle Grounds window has no transition command),
     * but it must contain multiple real level/behavior receipts. */
    const bool source_event_window = trace.have_record && trace.tick_count == 1u
    ...
    trace.file = NULL;
    if (!ok) {
        fprintf(stderr, "script_events_route_failed errors=%u first_error_status=%u message=%s\n",
                state.errors, state.first_error_status,
                state.first_error_message[0] ? state.first_error_message : "(none)");
        return false;
    }
    printf("c_script_events_route_recorded shard=0x%016" PRIx64
           " path=%s records=%" PRIu64 " ticks=%" PRIu64
           " event_counts=%u,%u,%u,%u,%u coverage=0x%016" PRIx64 "\n",
           SCRIPT_EVENTS_ROUTE_SHARD_ID, trace_path, trace.records, trace.last_tick,
           trace.records_by_event[0], trace.records_by_event[1], trace.records_by_event[2],
           trace.records_by_event[3], trace.records_by_event[4],
...
int main(int argc, char **argv) {
    if (argc != 3) {
        fprintf(stderr, "usage: sm64-modern-script-events-route-contract TRACE SAVE_DIRECTORY\n");
        return 2;
    }
