...
private enum Phase85COError: Error, CustomStringConvertible {
    case invalid(String)
...
private func require(_ condition: @autoclosure () -> Bool, _ message: String) throws {
    guard condition() else { throw Phase85COError.invalid(message) }
}
...
private func validateRoute(_ records: [SM64OracleTraceRecord]) throws {
    ...
    try require(sign?.values == [0x35, signOnWall, 0], "bhvSignOnWall mapping mismatch")
    for subject in 17...20 {
        let coin = record(records, domain: 12, kind: 4, subject: UInt64(subject), id: 4, tick: 1)
        try require(coin?.values == [0x35, oneCoin, 0], "bhvOneCoin subject-\(subject) mismatch")
    }
    for subject in [62, 63] {
        let child = record(records, domain: 12, kind: 4, subject: UInt64(subject), id: 4, tick: 1)
        try require(child?.values == [0x35, floorTrapChild, 0], "floor-trap child subject-\(subject) mismatch")
    }
...
private func writeProjection(_ input: String, _ output: String) throws {
    ...
    print("phase85co_swift_projection_written=1 records=\(records.count)")
}

private func audit(_ paths: ArraySlice<String>) throws {
    guard paths.count == 5 else { throw Phase85COError.invalid("audit requires five traces") }
    ...
    for index in 1..<values.count {
        ...
        if let offset = firstDifference(values[0].0, values[index].0) {
            throw Phase85COError.invalid("full-trace divergence trace=\(index) offset=\(offset)")
        }
    }
    print("phase85co_full_parity records=\(values[0].2.count) sequence12_tick=63 pcm_records=\(expectedPCMRecords) c_swift_asan_release_rerun_byte_match=1")
}
...
private func reject(_ path: String, _ errors: SM64OracleTraceCodecError...) throws {
    do {
        ...
        throw Phase85COError.invalid("negative fence accepted \(path)")
...
private func tamper(_ input: String, _ output: String) throws {
    ...
    print("phase85co_tamper_fence=1")
}
...
private func partial(_ input: String, _ output: String) throws {
    ...
    print("phase85co_partial_fence=1")
}

@main
struct SM64ModernPhase85COAudioFullRerunMapped {
    static func main() {
        do {
            let arguments = Array(CommandLine.arguments.dropFirst())
            guard let mode = arguments.first else { throw Phase85COError.invalid("missing mode") }
            switch mode {
            case "write":
                guard arguments.count == 3 else { throw Phase85COError.invalid("write requires C SWIFT") }
                try writeProjection(arguments[1], arguments[2])
            case "audit":
                try audit(arguments.dropFirst())
            case "tamper":
                guard arguments.count == 3 else { throw Phase85COError.invalid("tamper requires input output") }
                try tamper(arguments[1], arguments[2])
            case "partial":
                guard arguments.count == 3 else { throw Phase85COError.invalid("partial requires input output") }
                try partial(arguments[1], arguments[2])
            case "fixture":
                guard arguments.count == 3 else { throw Phase85COError.invalid("fixture requires input marker") }
                ...
                print("phase85co_fixture_only_fence=1")
                throw Phase85COError.invalid("fixture-only evidence rejected")
            case "single":
                guard arguments.count == 3 else { throw Phase85COError.invalid("single requires two paths") }
                guard URL(fileURLWithPath: arguments[1]).standardizedFileURL == URL(fileURLWithPath: arguments[2]).standardizedFileURL else {
                    throw Phase85COError.invalid("single-artifact fence did not reject distinct paths")
                }
                print("phase85co_single_artifact_fence=1")
                throw Phase85COError.invalid("single-artifact evidence rejected")
            default:
                throw Phase85COError.invalid("unknown mode \(mode)")
            }
        } catch {
            FileHandle.standardError.write(Data("phase85co_audio_full_rerun_failed=\(error)\n".utf8))
            exit(1)
        }
    }
}
