...
@main
struct SM64BlackSmokeUpwardSmoke {
    static func main() {
        let output = SM64BlackSmokeUpwardBehavior.update(.init(position: .init(x: 10, y: 20, z: -4), scale: 2, timer: 0))
        precondition(output.position == .init(x: 10, y: 20, z: -4) && output.scale == 2 && output.spawnChild && !output.shouldDeactivate)
        ...
        fingerprint = hash(fingerprint, output.spawnChild ? 1 : 0)
        fingerprint = hash(fingerprint, output.shouldDeactivate ? 1 : 0)
        print(String(format: "blackSmokeUpwardFingerprint=0x%016llx", fingerprint))
        print("SM64 Modern upward black smoke smoke passed")
    }
}
