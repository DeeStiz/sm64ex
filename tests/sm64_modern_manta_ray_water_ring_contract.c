...
int main(void) {
    ...
    fingerprint = row(fingerprint, 0, 1, 150, 0, 0);
    fingerprint = row(fingerprint, 1, 0, 150, 0, 1);
    fingerprint = row(fingerprint, 1, 22, 140, 1, 0);
    printf("mantaRayWaterRingFingerprint=0x%016llx\n", (unsigned long long)fingerprint);
    puts("SM64 Modern Manta Ray water ring C contract passed");
    return 0;
}
