...
/*
 * This harness follows the ordinary owner-thread lifecycle and only retains
 * the authored Snowman wind subject after the Castle Inside painting route
 * reaches Snowman's Land area 1.  The source observer is the only route
 * receipt producer; this file does not select by coordinates or manufacture a
 * schema-4 record.
 */
...
struct TraceFile {
    ...
    uint64_t first_tick;
    uint64_t last_tick;
    uint64_t last_domain_tick[SM64_MODERN_ORACLE_TRACE_DOMAIN_COUNT];
    uint32_t last_domain_sequence[SM64_MODERN_ORACLE_TRACE_DOMAIN_COUNT];
...
static bool route_record(const SM64ModernOracleTraceRecordV1 *record) {
    if (!record
        || record->subject_id != SM64_MODERN_SNOWMAN_WIND_ROUTE_OWNER_ID) {
        return false;
    }
    if (record->domain == SM64_MODERN_ORACLE_DOMAIN_SCRIPT
        && record->record_kind == SM64_MODERN_ORACLE_RECORD_EVENT
        && (record->record_id == SM64_MODERN_SNOWMAN_WIND_ROUTE_RECORD_STATE
            || record->record_id
    ...
    return record->domain == SM64_MODERN_ORACLE_DOMAIN_EFFECT
        ...
        && record->record_id == SM64_MODERN_SNOWMAN_WIND_ROUTE_RECORD_EFFECT;
}
...
static SM64ModernStatus trace_write_record(
    ...
    if (route_record(record)) {
        if (record->flags != SM64_MODERN_SNOWMAN_WIND_ROUTE_FLAGS
            || record->value_count != SM64_MODERN_ORACLE_TRACE_VALUE_CAPACITY) {
            trace->failures++;
            return SM64_MODERN_STATUS_PARITY_DIVERGED;
        }
    ...
    if (trace->domain_seen[domain]
        && (record->simulation_tick < trace->last_domain_tick[domain]
            || (record->simulation_tick == trace->last_domain_tick[domain]
                && record->sequence
                    <= trace->last_domain_sequence[domain]))) {
        trace->failures++;
        return SM64_MODERN_STATUS_PARITY_DIVERGED;
    }
    trace->domain_seen[domain] = true;
    trace->last_domain_tick[domain] = record->simulation_tick;
    trace->last_domain_sequence[domain] = record->sequence;
...
static SM64ModernLifecycleConfigV1 lifecycle_config(const char *save_directory) {
    ...
    snprintf(config.config_file, sizeof(config.config_file), "%s",
             "sm64-modern-snowman-wind-route.cfg");
    snprintf(config.window_title, sizeof(config.window_title), "%s",
             "SM64 Modern Snowman Wind Route");
    return config;
}
...
static SM64ModernOracleTraceConfigV1 oracle_config(uint64_t timebase_fingerprint) {
    ...
    config.build_fingerprint = hash_string(
        "sm64-modern-snowman-wind-route-build-v1");
    config.content_fingerprint = hash_string(
        "data/behavior_data.c:3478-3484|"
        "src/game/behaviors/sl_snowman_wind.inc.c|"
        "levels/castle_inside/script.c:114-116|"
        "levels/sl/script.c:31-37,67-87|"
        "bhvSLSnowmanWind|MODEL_NONE|script_func_local_3:1|"
        "source_observer");
    ...
    config.configuration_fingerprint = hash_string(
        ...
        "castle_painting_nodes=0x24,0x25,0x26;sl_area=1;"
        "script_func_local_3_ordinal=1;position=700,3428,700;yaw=30;"
        "parameter=0;shard=0xa98dae7d4d4559ab");
    config.initial_save_fingerprint = hash_string(
        "save=empty-us-slot-0;seed=0x7bd3187854922858");
    config.coverage_fingerprint = 0u;
    return config;
}

static struct Object *find_first_snowman_wind(uint32_t *out_count) {
    const BehaviorScript *target = segmented_to_virtual(bhvSLSnowmanWind);
...
static bool run_route(const char *trace_path, const char *save_directory) {
    ...
    struct Object *first_snowman_wind = NULL;
    uint32_t snowman_wind_count = 0;
    for (uint32_t step = 0; step < ROUTE_WARMUP_LIMIT; ++step) {
        ...
        if (gCurrLevelNum == LEVEL_SL && gCurrentArea != NULL
            ...
            first_snowman_wind = find_first_snowman_wind(&snowman_wind_count);
            if (first_snowman_wind != NULL && snowman_wind_count == 1u) break;
        }
    }

    if (first_snowman_wind == NULL || snowman_wind_count != 1u
        || gCurrLevelNum != LEVEL_SL || gCurrentArea == NULL
        || gCurrentArea->index != 1u) {
        fprintf(stderr,
                "snowman_wind_route_unreachable level=%d area=%d wind=%u\n",
                gCurrLevelNum,
    ...
    trace.selected_subject = sm64_modern_parity_object_slot(first_snowman_wind);
    ...
    sm64_modern_snowman_wind_route_reset();
    ...
    const SM64ModernSnowmanWindRouteReceiptV1 *receipt =
        sm64_modern_snowman_wind_route_last_receipt();
    const uint32_t expected_route_records =
        sm64_modern_snowman_wind_route_matches() * 4u;
    const bool receipt_is_canonical =
        receipt->source_identity == SM64_MODERN_SNOWMAN_WIND_ROUTE_SOURCE_ID
        && receipt->owner_identity == SM64_MODERN_SNOWMAN_WIND_ROUTE_OWNER_ID
        ...
        && receipt->flags == SM64_MODERN_SNOWMAN_WIND_ROUTE_FLAGS;
    ok = ok
        ...
        && trace.state_records
            == sm64_modern_snowman_wind_route_matches()
        && trace.motion_records
            == sm64_modern_snowman_wind_route_matches()
        && trace.collision_records
            == sm64_modern_snowman_wind_route_matches()
        ...
        && sm64_modern_snowman_wind_route_matches() > 0u
        && state.errors == 0u;
    if (!ok) {
        fprintf(stderr,
                "snowman_wind_route_debug oracle_end=%" PRIu32
                ...
                trace.collision_records,
                trace.effect_records,
                sm64_modern_snowman_wind_route_invocations(),
                sm64_modern_snowman_wind_route_matches(),
                sm64_modern_snowman_wind_route_selected_subject(),
    ...
    printf(
        "snowman_wind_route_debug oracle_end=0 result_status=0"
...
int main(int argc, char **argv) {
    ...
    if (!run_route(argv[1], argv[2])) {
        fprintf(stderr,
                "SM64 Modern Snowman wind route pair blocked "
                "reachability=0\n"
                "source_lifecycle=castle_inside_painting_nodes_0x24_0x25_0x26_to_sl_area1\n"
                "first_snowman_wind_receipt=absent sibling_substitution=0 "
                "coordinate_matching=0 fixture_only=0\n"
