...
@main
struct SM64FlameSmoke {
    static func main() {
        let output = SM64FlameBehavior.update(.init(position: .init(x: 10, y: 20, z: -4), timer: 0, animationState: 0))
        precondition(output.position == .init(x: 10, y: 20, z: -4) && output.animationState == 1 && output.interactionStatus == 0)
        ...
        fingerprint = hash(fingerprint, UInt64(bitPattern: Int64(output.interactionStatus)))
        print(String(format: "flameFingerprint=0x%016llx", fingerprint))
        print("SM64 Modern static flame smoke passed")
    }
}
