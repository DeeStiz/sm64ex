...
#define HEADER_BASE UINT64_C(0x4d465f00)
#define TEXTURE_BASE UINT64_C(0x4d466000)
...
static uint32_t policy_code(void) {
...
static uint64_t plan_fingerprint(void) {
    ...
    for (uint32_t index = 0; index < 3u; ++index) {
        ...
        hash = hash_u64(hash, policy_code());
    }
    return hash;
}
...
static SM64ModernStatus emit_record(uint64_t *trace_fingerprint, uint32_t *sequence,
    ...
    if (tamper && record_id == (HEADER_BASE | 2u)) values[7]++;
...
static int replay(struct Trace *trace, int tamper) {
    ...
    config.build_fingerprint = 0x4d301201u;
    config.content_fingerprint = 0x4d301202u;
    config.timebase_fingerprint = 0x4d301203u;
    config.configuration_fingerprint = 0x4d301204u;
    config.initial_save_fingerprint = 0x4d301205u;
    ...
    const uint64_t fingerprint = plan_fingerprint();
    ...
    uint64_t header[] = {
        2u, 3u, 5120u, 12288u, 1u, 3u,
        UINT64_C(0xa1f8ed42ee0984fc), fingerprint,
    };
    ...
    for (uint32_t index = 0; index < 3u; ++index) {
        uint64_t values[] = {
            sTextureIDs[index], index + 1u, 32u, 32u, 4096u,
            sUploadFingerprints[index], policy_code(), 1u,
        };
        status = emit_record(
    ...
    if (!tamper) {
        ...
        printf("marioFaceTextureUploadAdmissionRoute=2\n");
        printf("marioFaceTextureUploadAdmissionEntries=3\n");
        printf("marioFaceTextureUploadAdmissionSourceBytes=5120\n");
        printf("marioFaceTextureUploadAdmissionUploadBytes=12288\n");
        printf("marioFaceTextureUploadAdmissionPendingResidency=3\n");
        printf("marioFaceTextureUploadAdmissionPlanFingerprint=0x%016llx\n",
               (unsigned long long) fingerprint);
        printf("marioFaceTextureUploadAdmissionTraceFingerprint=0x%016llx\n",
               (unsigned long long) trace_fingerprint);
        printf("SM64 Modern Mario-face texture upload admission C replay passed entries=3\n");
        return 1;
    }
    if (end_status != SM64_MODERN_STATUS_PARITY_DIVERGED || first_divergence != 2u) return 0;
    printf("SM64 Modern Mario-face texture upload admission C divergence detected first_divergence=%u\n",
           first_divergence);
    return 1;
}
