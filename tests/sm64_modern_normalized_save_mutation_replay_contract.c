...
static void encode_save(
    ...
    bytes[0] = cap_level; bytes[1] = cap_area;
    put16(bytes, 2, (uint16_t)x); put16(bytes, 4, (uint16_t)y);
    put16(bytes, 6, (uint16_t)z); put32(bytes, 8, flags);
...
static uint64_t hash_state(
    ...
    uint8_t cap_level, uint8_t cap_area, int16_t x, int16_t y, int16_t z,
    const uint32_t *ages, uint16_t sound, uint8_t did_mutate
) {
    ...
    return h8(fingerprint, did_mutate);
}

static uint64_t hash_runtime(
    uint64_t fingerprint, uint32_t flags, const uint8_t *stars,
    uint8_t cap_level, uint8_t cap_area, int16_t x, int16_t y, int16_t z,
    const uint32_t *ages, uint16_t sound, uint8_t save_modified,
    uint8_t menu_modified
) {
    uint8_t save[SAVE_BYTES], menu[MENU_BYTES];
    encode_save(save, flags, stars, cap_level, cap_area, x, y, z);
    encode_menu(menu, ages, sound);
    ...
    return h8(fingerprint, menu_modified);
}

int main(void) {
    uint8_t stars[25] = {0};
    stars[0] = 0x04;
    const uint32_t ages[4] = {
    ...
    uint8_t save_modified = 0, menu_modified = 0;
    ...
    // Paused-domain operation is a strict no-op.
    fingerprint = hash_state(
        fingerprint, flags, stars, cap_level, cap_area, x, y, z,
        ages, sound, 0
    );
    ...
    // A move without CAP_ON_GROUND fails closed and leaves bytes untouched.
    fingerprint = hash_state(
        fingerprint, flags, stars, cap_level, cap_area, x, y, z,
        ages, sound, 0
    );

    // The runtime's normalized commit path sets the 50-star door bit and
    // clears both dirty bits only after the atomic write succeeds.
    flags |= (UINT32_C(1) << 20) | FILE_EXISTS;
    fingerprint = hash_runtime(
        fingerprint, flags, stars, cap_level, cap_area, x, y, z,
        ages, sound, 1, 0
    );
    save_modified = 0; menu_modified = 0;
    fingerprint = hash_runtime(
        fingerprint, flags, stars, cap_level, cap_area, x, y, z,
        ages, sound, save_modified, menu_modified
    );

    printf(
        "normalizedSaveMutationReplayFingerprint=0x%016llx\n",
