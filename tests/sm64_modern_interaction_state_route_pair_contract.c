...
/*
 * C owner for the generated oracle_hook|interaction_state row.  The native
 * Mario interaction fields are retained at the schema-4 stream boundary;
 * this harness does not construct collisions or manufacture an interaction.
 */
#define INTERACTION_ROUTE_SHARD_ID UINT64_C(0x3e1cdaca08b21f54)
#define INTERACTION_ROUTE_INPUT_SEED UINT64_C(0xee187b29391cf62c)
#define INTERACTION_ROUTE_SAVE_SEED UINT64_C(0x7bc40eed25255955)
#define INTERACTION_ROUTE_TRACE_STEPS 2u
#define INTERACTION_ROUTE_RECORD_FIRST SM64_MODERN_FIELD_INTERACTION_TYPES
#define INTERACTION_ROUTE_RECORD_LAST SM64_MODERN_FIELD_INTERACTION_HEAL_COUNTER
#define INTERACTION_ROUTE_RECORD_COUNT \
    (INTERACTION_ROUTE_RECORD_LAST - INTERACTION_ROUTE_RECORD_FIRST + 1u)
#define INTERACTION_ROUTE_FNV_OFFSET UINT64_C(1469598103934665603)
#define INTERACTION_ROUTE_FNV_PRIME UINT64_C(1099511628211)
...
struct TraceFile {
    ...
    bool have_record;
    bool retained_ids[INTERACTION_ROUTE_RECORD_COUNT];
    uint32_t tick_count;
    uint32_t failures;
};
...
static uint64_t hash_u64(uint64_t hash, uint64_t value) {
    for (uint32_t byte = 0; byte < 8u; ++byte) {
        ...
        hash *= INTERACTION_ROUTE_FNV_PRIME;
    }
    return hash;
}

static uint64_t hash_string(const char *value) {
    uint64_t hash = INTERACTION_ROUTE_FNV_OFFSET;
...
static uint64_t retained_coverage_fingerprint(const struct TraceFile *trace) {
    uint64_t hash = INTERACTION_ROUTE_FNV_OFFSET;
    uint32_t count = 0;
    for (uint32_t index = 0; index < INTERACTION_ROUTE_RECORD_COUNT; ++index) {
        if (!trace->retained_ids[index]) {
            continue;
        }
        hash = hash_u64(hash, SM64_MODERN_ORACLE_DOMAIN_INTERACTION);
        hash = hash_u64(hash, 0u);
        hash = hash_u64(hash, INTERACTION_ROUTE_RECORD_FIRST + index);
        count++;
    }
...
static SM64ModernStatus trace_write_record(
     ...
     * this generated interaction row. */
    if (record->domain != SM64_MODERN_ORACLE_DOMAIN_INTERACTION) {
        return SM64_MODERN_STATUS_OK;
    }
    if (record->record_kind != SM64_MODERN_ORACLE_RECORD_STATE
        || record->record_id < INTERACTION_ROUTE_RECORD_FIRST
        || record->record_id > INTERACTION_ROUTE_RECORD_LAST
        || record->subject_id != 0u
        || record->value_count != 1u
        || record->simulation_tick < 2u
        || record->simulation_tick > 3u
        || record->record_id != INTERACTION_ROUTE_RECORD_FIRST + record->sequence) {
        trace->failures++;
    ...
    const uint32_t index = (uint32_t) record->record_id
        - INTERACTION_ROUTE_RECORD_FIRST;
...
static SM64ModernLifecycleConfigV1 make_lifecycle_config(const char *save_directory) {
    ...
    snprintf(config.config_file, sizeof(config.config_file), "%s",
             "sm64-modern-interaction-state-route.cfg");
    snprintf(config.window_title, sizeof(config.window_title), "%s",
             "SM64 Modern Interaction State Route");
    return config;
}
...
static SM64ModernOracleTraceConfigV1 make_oracle_config(uint64_t timebase_fingerprint) {
    ...
    config.mode = SM64_MODERN_ORACLE_TRACE_RECORD;
    config.build_fingerprint = hash_string("sm64-modern-interaction-state-route-build-v1");
...
static bool record_route(const char *trace_path, const char *save_directory) {
    ...
    if (ok) {
        ...
        fprintf(stderr, "interaction_state_route_init status=%u oracle=%u\n",
                init_status, sm64_modern_oracle_trace_status());
        ok = ok && init_status == SM64_MODERN_STATUS_OK;
        sm64_modern_oracle_trace_end_tick();
        for (uint32_t index = 0; ok && index < INTERACTION_ROUTE_TRACE_STEPS; ++index) {
            const SM64ModernStatus status = lifecycle.step();
            fprintf(stderr,
                    "interaction_state_route_step index=%u status=%u oracle=%u parity=%u\n",
    ...
    fprintf(stderr,
            "interaction_state_route_debug oracle_end=%u result_status=%u actual=%" PRIu64
    ...
    ok = ok && oracle_end == SM64_MODERN_STATUS_OK
        ...
        && trace.records == INTERACTION_ROUTE_RECORD_COUNT * INTERACTION_ROUTE_TRACE_STEPS
        && trace.tick_count == 1u
        && trace.failures == 0u
        && state.errors == 0u
    ...
    if (!ok) {
        fprintf(stderr,
                "interaction_state_route_failed errors=%u first_error_status=%u message=%s\n",
                state.errors, state.first_error_status,
                state.first_error_message[0] ? state.first_error_message : "(none)");
        return false;
    }
    printf("c_interaction_state_route_recorded shard=0x%016" PRIx64
           " path=%s records=%" PRIu64 " ticks=%" PRIu64
           " coverage=0x%016" PRIx64 "\n",
           INTERACTION_ROUTE_SHARD_ID, trace_path, trace.records, trace.last_tick,
...
int main(int argc, char **argv) {
    if (argc != 3) {
        fprintf(stderr,
                "usage: sm64-modern-interaction-state-route-contract TRACE SAVE_DIRECTORY\n");
        return 2;
    }
