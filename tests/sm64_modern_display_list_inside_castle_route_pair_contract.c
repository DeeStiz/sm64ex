...
static uint64_t resource_fingerprint(
    const SM64ModernDisplayListInsideCastleRoutePacketV1 *packet) {
    uint64_t hash = ROUTE_FNV_OFFSET;
...
static bool valid_record(const SM64ModernOracleTraceRecordV1 *record) {
    return record
        ...
        && record->subject_id == SM64_MODERN_DISPLAY_LIST_INSIDE_CASTLE_ROUTE_SHARD_ID
        && record->record_id == SM64_MODERN_DISPLAY_LIST_INSIDE_CASTLE_ROUTE_EVENT
...
static SM64ModernStatus trace_write_header(
    ...
    if (!trace || !config
        ...
        || config->build_fingerprint != hash_string(
            "sm64-modern-display-list-inside-castle-route-build-v1")
        || config->content_fingerprint != hash_string(
            "levels/castle_inside/areas/2/3/model.inc.c|"
            "inside_castle_seg7_dl_07043A68|render_packet")
        ...
        || config->initial_save_fingerprint != hash_string(
            "save=empty-us-slot-0;seed=0x2c483fa0516b9079")
...
static bool write_packet_sidecar(const char *path) {
    const SM64ModernDisplayListInsideCastleRoutePacketV1 *packet =
        sm64_modern_display_list_inside_castle_route_last_packet();
...
static int run_route(const char *trace_path, const char *packet_path) {
    ...
    sm64_modern_display_list_inside_castle_route_reset();
    ...
    for (uint64_t tick = 1u; tick <= ROUTE_TICKS; ++tick) {
        ...
        status = sm64_modern_display_list_inside_castle_route_observe_scene_graph_append(
            inside_castle_seg7_dl_07043B48, 1u);
        sm64_modern_oracle_trace_end_tick();
        if (status != SM64_MODERN_STATUS_OK) {
            fprintf(stderr,
                "display_list_inside_castle_route_step tick=%" PRIu64
                " status=%" PRIu32 "\n", tick, status);
    ...
    const bool packet_values_are_canonical =
        packet->source_identity == SM64_MODERN_DISPLAY_LIST_INSIDE_CASTLE_ROUTE_SOURCE_ID
        && packet->owner_identity == SM64_MODERN_DISPLAY_LIST_INSIDE_CASTLE_ROUTE_OWNER_ID
        && packet->word_count == 6u
        && packet->drawing_layer == 1u
        && packet->triangle_count == 2u
        && packet->flags == 3u
        && packet->resource_ids[0] == SM64_MODERN_DISPLAY_LIST_INSIDE_CASTLE_ROUTE_TEXTURE_RESOURCE_ID
        && packet->resource_ids[3] == SM64_MODERN_DISPLAY_LIST_INSIDE_CASTLE_ROUTE_VERTEX_RESOURCE_ID;
    if (status != SM64_MODERN_STATUS_OK
        || trace.failures != 0u
        || trace.records != ROUTE_TICKS
        || sm64_modern_display_list_inside_castle_route_matches() != ROUTE_TICKS
        || sm64_modern_display_list_inside_castle_route_invocations() != ROUTE_TICKS
        || !packet_values_are_canonical
        || !write_packet_sidecar(packet_path)) {
        fprintf(stderr,
            "display_list_inside_castle_route_debug oracle_end=%" PRIu32
            " trace_status=%" PRIu32
            " records=%" PRIu64
            " invocations=%" PRIu32
            " matches=%" PRIu32
            ...
            trace.records,
            sm64_modern_display_list_inside_castle_route_invocations(),
            sm64_modern_display_list_inside_castle_route_matches(),
            trace.failures);
        return 5;
    }

    printf(
        "c_display_list_inside_castle_route_recorded shard=0x%016" PRIx64
        ...
        SM64_MODERN_DISPLAY_LIST_INSIDE_CASTLE_ROUTE_SHARD_ID,
        trace.records,
        packet->source_identity,
        packet->owner_identity,
        packet->word_count,
    ...
    printf(
        "display_list_inside_castle_route_debug oracle_end=0 result_status=0 records=%" PRIu64
    ...
    const uint32_t matches =
        sm64_modern_display_list_inside_castle_route_matches();
    sm64_modern_display_list_inside_castle_route_observe_scene_graph_append(NULL, 1u);
    sm64_modern_display_list_inside_castle_route_observe_scene_graph_append(
        sm64_modern_inside_castle_display_list_07043A68_source(), 1u);
    if (sm64_modern_display_list_inside_castle_route_matches() != matches) {
        fprintf(stderr, "display_list_inside_castle_route_owner_fence_failed=1\n");
        return 6;
    }
    printf("display_list_inside_castle_route_owner_fence=1 pointer_free_packet=1\n");
    return 0;
}
