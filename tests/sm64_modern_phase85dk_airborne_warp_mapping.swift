...
private enum Phase85DKError: Error, CustomStringConvertible {
    case invalid(String)
...
private let expectedAirborneWarp: UInt64 = 0x3e6a_f9ed_47c5_9929

private func require(_ condition: @autoclosure () -> Bool, _ message: String) throws {
    guard condition() else { throw Phase85DKError.invalid(message) }
}
...
private func sourceCheck() throws {
    ...
    try require(
        ...
        "authored airborne-warp source entry missing"
    )
    try require(
        source.contains(
            "OBJECT(/*model*/ MODEL_NONE, /*pos*/ -1024,  900,   717, "
        ...
        "adjacent airborne-death source entry missing"
    )
    try require(
        source.contains(
            "OBJECT(/*model*/ MODEL_NONE, /*pos*/ -1024,  900,   717, "
    ...
    try require(
        owner.contains("SOURCE_BEHAVIOR_IDENTITY(bhvAirborneWarp)"),
        "airborne-warp semantic owner mapping missing"
    )
    try require(
        fnv("bhvAirborneWarp") == expectedAirborneWarp,
        "airborne-warp semantic FNV identity changed"
    )
}
...
private func validateRoute(_ records: [SM64OracleTraceRecord]) throws {
    ...
    let airborneWarp = record(records, domain: 3, kind: 1, subject: 39, id: 400, tick: 1)
    try require(
        airborneWarp?.values == [expectedAirborneWarp],
        "subject-39 airborne-warp semantic identity mismatch"
    )
    let airborneWarpYaw = record(records, domain: 3, kind: 1, subject: 39, id: 407, tick: 1)
    try require(
        airborneWarpYaw?.values == [0, 0xffff_8000, 0],
        "subject-39 airborne-warp yaw provenance missing"
    )
}
...
private func requireInvalid(_ path: String, _ expected: SM64OracleTraceCodecError...) throws {
    do {
        ...
        throw Phase85DKError.invalid("negative fence accepted \(path)")
...
private func tamper(_ input: String, _ output: String) throws {
    ...
    print("phase85dk_tamper_fence=1")
}
...
private func partial(_ input: String, _ output: String) throws {
    ...
    print("phase85dk_partial_fence=1")
}
...
private func fixture(_ trace: String, _ marker: String) throws {
    ...
    print("phase85dk_fixture_only_fence=1")
    throw Phase85DKError.invalid("fixture-only evidence rejected")
}

@main
struct SM64ModernPhase85DKAirborneWarpMapping {
    static func main() {
        do {
            let arguments = Array(CommandLine.arguments.dropFirst())
            guard let mode = arguments.first else {
                throw Phase85DKError.invalid(
            ...
            case "write":
                ...
                print("phase85dk_swift_projection_written=1 records=\(c.2.count)")
            case "audit":
                ...
                print(
                    "phase85dk_full_parity records=\(values[0].2.count) sequence12_tick=63 "
                        + "pcm_records=\(expectedPCMRecords) airborne_warp_identity=0x"
                        + String(expectedAirborneWarp, radix: 16)
            ...
            case "single":
                ...
                print("phase85dk_single_artifact_fence=1")
                throw Phase85DKError.invalid("single-artifact evidence rejected")
            default:
                throw Phase85DKError.invalid("unknown mode \(mode)")
            }
        } catch {
            FileHandle.standardError.write(
                Data("phase85dk_airborne_warp_mapping_failed=\(error)\n".utf8)
            )
            exit(1)
        }
    }
