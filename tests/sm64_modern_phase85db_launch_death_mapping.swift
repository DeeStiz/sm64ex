...
private enum Phase85DBError: Error, CustomStringConvertible {
    case invalid(String)
...
private func require(_ condition: @autoclosure () -> Bool, _ message: String) throws {
    guard condition() else { throw Phase85DBError.invalid(message) }
}
...
private func sourceCheck() throws {
    ...
    try require(
        source.contains(
            "OBJECT(/*model*/ MODEL_NONE, /*pos*/ -5513,  512, -4324, "
                + "/*angle*/ 0, -135, 0, /*behParam*/ 0x00250000, /*beh*/ bhvLaunchDeathWarp),"
        ),
        "authored -135-degree launch-death source entry missing"
    )
    try require(
        source.contains(
            "OBJECT(/*model*/ MODEL_NONE, /*pos*/ -5513,  512, -4324, "
                + "/*angle*/ 0, -135, 0, /*behParam*/ 0x00240000, /*beh*/ bhvLaunchStarCollectWarp),"
        ),
        "adjacent launch-star source entry missing"
    )
    ...
    try require(
        ...
        "launch-death semantic owner mapping missing"
    )
    try require(
...
private func requireInvalid(_ path: String, _ expected: SM64OracleTraceCodecError...) throws {
    do {
        ...
        throw Phase85DBError.invalid("negative fence accepted \(path)")
...
private func tamper(_ input: String, _ output: String) throws {
    ...
    print("phase85db_tamper_fence=1")
}
...
private func partial(_ input: String, _ output: String) throws {
    ...
    print("phase85db_partial_fence=1")
}
...
private func fixture(_ trace: String, _ marker: String) throws {
    ...
    print("phase85db_fixture_only_fence=1")
    throw Phase85DBError.invalid("fixture-only evidence rejected")
}

@main
struct SM64ModernPhase85DBLaunchDeathMapping {
    static func main() {
        do {
            let arguments = Array(CommandLine.arguments.dropFirst())
            guard let mode = arguments.first else {
                throw Phase85DBError.invalid(
            ...
            case "write":
                ...
                print("phase85db_swift_projection_written=1 records=\(c.2.count)")
            case "audit":
                ...
                print(
                    "phase85db_full_parity records=\(values[0].2.count) sequence12_tick=63 "
                        + "pcm_records=\(expectedPCMRecords) launch_death_identity=0x"
                        + String(expectedLaunchDeathWarp, radix: 16)
            ...
            case "single":
                ...
                print("phase85db_single_artifact_fence=1")
                throw Phase85DBError.invalid("single-artifact evidence rejected")
            default:
                throw Phase85DBError.invalid("unknown mode \(mode)")
            }
        } catch {
            FileHandle.standardError.write(
                Data("phase85db_launch_death_mapping_failed=\(error)\n".utf8)
            )
            exit(1)
        }
    }
