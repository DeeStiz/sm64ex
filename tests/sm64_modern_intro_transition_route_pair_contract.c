...
/*
 * The authored intro level script remains the sole transition producer.  The
 * callback filters the existing schema-4 script event ID 3; it does not call
 * the transition implementation, execute an opcode, or add a second record.
 */
#define INTRO_TRANSITION_ROUTE_SHARD_ID UINT64_C(0x9a0f7b4f7ecf6c41)
#define INTRO_TRANSITION_ROUTE_INPUT_SEED UINT64_C(0x6c1f8a943cb27d50)
#define INTRO_TRANSITION_ROUTE_SAVE_SEED UINT64_C(0x2e7fdb4a0c5689b1)
#define INTRO_TRANSITION_ROUTE_STEPS 320u
#define INTRO_TRANSITION_ROUTE_FIRST_TICK UINT64_C(311)
#define INTRO_TRANSITION_ROUTE_SECOND_TICK UINT64_C(391)
#define INTRO_TRANSITION_ROUTE_FIRST_HASH UINT64_C(0xb9e77a797c34bb9e)
#define INTRO_TRANSITION_ROUTE_SECOND_HASH UINT64_C(0x0eb91077dbe72aa4)
#define INTRO_TRANSITION_ROUTE_COVERAGE UINT64_C(0x8fc5fa3c2cd26867)
#define INTRO_TRANSITION_ROUTE_FNV_OFFSET UINT64_C(1469598103934665603)
#define INTRO_TRANSITION_ROUTE_FNV_PRIME UINT64_C(1099511628211)
...
static uint64_t hash_u64(uint64_t hash, uint64_t value) {
    for (uint32_t byte = 0; byte < 8u; ++byte) {
        ...
        hash *= INTRO_TRANSITION_ROUTE_FNV_PRIME;
    }
    return hash;
}

static uint64_t hash_string(const char *value) {
    uint64_t hash = INTRO_TRANSITION_ROUTE_FNV_OFFSET;
...
static bool exact_target_record(
    const SM64ModernOracleTraceRecordV1 *record,
    uint32_t index) {
    static const uint64_t values[2][5] = {
        { UINT64_C(1), UINT64_C(16), UINT64_C(0), UINT64_C(0), UINT64_C(0) },
        { UINT64_C(8), UINT64_C(20), UINT64_C(0), UINT64_C(0), UINT64_C(0) },
    };
    static const uint64_t ticks[2] = {
        INTRO_TRANSITION_ROUTE_FIRST_TICK,
        INTRO_TRANSITION_ROUTE_SECOND_TICK,
    };
    static const uint32_t sequences[2] = { 2u, 5u };
    static const uint64_t hashes[2] = {
        INTRO_TRANSITION_ROUTE_FIRST_HASH,
        INTRO_TRANSITION_ROUTE_SECOND_HASH,
    };
    if (!record || index >= 2u
        || record->simulation_tick != ticks[index]
        || record->domain != SM64_MODERN_ORACLE_DOMAIN_SCRIPT
        || record->record_kind != SM64_MODERN_ORACLE_RECORD_EVENT
        || record->subject_id != UINT64_C(1)
        || record->record_id != SM64_MODERN_ORACLE_SCRIPT_EVENT_LEVEL_TRANSITION
        || record->sequence != sequences[index]
        || record->value_count != 5u
        || record->canonical_hash != hashes[index]) {
        return false;
    }
    for (uint32_t value = 0; value < 5u; ++value) {
        if (record->values[value] != values[index][value]) {
            return false;
        }
    }
    return true;
...
static SM64ModernStatus trace_write_record(
    ...
    /* All other domains and script IDs remain native producer side effects;
     * this route retains only the authored transition ID 3. */
    ...
    if (index >= 2u || !exact_target_record(record, index)) {
        trace->failures++;
    ...
    trace->target_hashes[index] = record->canonical_hash;
    trace->records++;
    return SM64_MODERN_STATUS_OK;
}
...
static SM64ModernOracleTraceConfigV1 make_config(uint64_t timebase_fingerprint) {
    ...
    config.mode = SM64_MODERN_ORACLE_TRACE_RECORD;
    config.build_fingerprint = hash_string("sm64-modern-intro-transition-route-build-v2");
    ...
    config.initial_save_fingerprint = hash_u64(
        INTRO_TRANSITION_ROUTE_FNV_OFFSET, INTRO_TRANSITION_ROUTE_SAVE_SEED);
    return config;
}
...
int main(int argc, char **argv) {
    if (argc != 3) {
        fprintf(stderr,
                "usage: sm64-modern-intro-transition-route-contract TRACE SAVE_DIRECTORY\n");
        return 2;
    }
    ...
    snprintf(lifecycle_config.save_directory,
             sizeof(lifecycle_config.save_directory), "%s", argv[2]);
    snprintf(lifecycle_config.config_file,
             sizeof(lifecycle_config.config_file), "%s",
             "sm64-modern-intro-transition-route.cfg");
    snprintf(lifecycle_config.window_title,
             sizeof(lifecycle_config.window_title), "%s",
             "SM64 Modern Intro Transition Route");

    if (ok) {
        sm64_modern_oracle_trace_begin_tick();
        const SM64ModernStatus status = lifecycle.initialize(&lifecycle_config, &platform);
        ok = status == SM64_MODERN_STATUS_OK;
        sm64_modern_oracle_trace_end_tick();
        for (uint32_t index = 0; ok && index < INTRO_TRANSITION_ROUTE_STEPS; ++index) {
            sm64_modern_oracle_trace_begin_tick();
    ...
    const bool target_pair = trace.records == 2u
        && trace.target_hashes[0] == INTRO_TRANSITION_ROUTE_FIRST_HASH
        && trace.target_hashes[1] == INTRO_TRANSITION_ROUTE_SECOND_HASH;
    const SM64ModernOracleTraceConfigV1 finalized_config = {
        ...
        .coverage_fingerprint = INTRO_TRANSITION_ROUTE_COVERAGE,
    };
    ok = ok && oracle_end == SM64_MODERN_STATUS_OK
        ...
        && state.errors == 0u
        && target_pair
        && rewrite_header(&trace, &finalized_config);

    sm64_modern_uninstall_input_api();
    ok = fclose(trace.file) == 0 && ok;
    fprintf(stderr,
            "intro_transition_route_debug oracle_end=%u result_status=%u "
            "actual=%" PRIu64 " retained=%u target_ticks=311,391 "
            "target_hashes=0x%016" PRIx64 ",0x%016" PRIx64 " "
            "failures=%u errors=%u coverage=0x%016" PRIx64 "\n",
            oracle_end, result.status, result.actual_records, trace.records,
            trace.target_hashes[0], trace.target_hashes[1], trace.failures,
            state.errors, INTRO_TRANSITION_ROUTE_COVERAGE);
    printf("intro_transition_route_recorded shard=0x%016" PRIx64
           " source=levels/intro/script.c entry=level_intro_entry_1 steps=%u "
           "records=%u ticks=311,391 "
           "hashes=0x%016" PRIx64 ",0x%016" PRIx64
           " coverage=0x%016" PRIx64 " owner_thread=%u\n",
           INTRO_TRANSITION_ROUTE_SHARD_ID, INTRO_TRANSITION_ROUTE_STEPS,
           trace.records, trace.target_hashes[0], trace.target_hashes[1],
           INTRO_TRANSITION_ROUTE_COVERAGE, trace.have_owner_thread ? 1u : 0u);
    if (!ok) return 1;
    puts("intro_transition_route_reachability passed transition_records=2");
    return 0;
}
