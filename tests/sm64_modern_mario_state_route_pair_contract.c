...
/*
 * This harness owns the C side of the generated oracle_hook|mario_state row.
 * It executes the real lifecycle and keeps the complete Mario state domain at
 * the stream boundary.  Initialization is deliberately its own oracle tick:
 * save/audio records emitted by lifecycle.initialize are real records, but
 * they must not poison the gameplay parity status by being written outside a
 * tick.  The row coverage is finalized from the retained domain-2 records
 * after the run; no fixture records or header rewrite can create a record.
 */
#define MARIO_STATE_ROUTE_SHARD_ID UINT64_C(0x88d04246f94ce9f8)
#define MARIO_STATE_ROUTE_INPUT_SEED UINT64_C(0x6b39a1052551fd60)
#define MARIO_STATE_ROUTE_SAVE_SEED UINT64_C(0x59a2b6ab37c5b109)
#define MARIO_STATE_ROUTE_TRACE_STEPS 2u
#define MARIO_STATE_ROUTE_KEY_A UINT32_C(0x26)
#define MARIO_STATE_ROUTE_RECORD_FIRST SM64_MODERN_FIELD_MARIO_INPUT
#define MARIO_STATE_ROUTE_RECORD_LAST SM64_MODERN_FIELD_MARIO_FRAMES_SINCE_B
#define MARIO_STATE_ROUTE_RECORD_COUNT \
    (MARIO_STATE_ROUTE_RECORD_LAST - MARIO_STATE_ROUTE_RECORD_FIRST + 1u)
#define MARIO_STATE_ROUTE_FNV_OFFSET UINT64_C(1469598103934665603)
#define MARIO_STATE_ROUTE_FNV_PRIME UINT64_C(1099511628211)
...
struct TraceFile {
    ...
    bool have_record;
    bool retained_ids[MARIO_STATE_ROUTE_RECORD_COUNT];
    uint64_t first_action;
    uint64_t second_action;
    uint32_t action_records;
    uint32_t failures;
};
...
static uint64_t hash_u64(uint64_t hash, uint64_t value) {
    for (uint32_t byte = 0; byte < 8u; ++byte) {
        ...
        hash *= MARIO_STATE_ROUTE_FNV_PRIME;
    }
    return hash;
}

static uint64_t hash_string(const char *value) {
    uint64_t hash = MARIO_STATE_ROUTE_FNV_OFFSET;
...
static uint64_t retained_coverage_fingerprint(const struct TraceFile *trace) {
    uint64_t hash = MARIO_STATE_ROUTE_FNV_OFFSET;
    uint32_t count = 0;
    for (uint32_t index = 0; index < MARIO_STATE_ROUTE_RECORD_COUNT; ++index) {
        if (!trace->retained_ids[index]) {
            continue;
        }
        hash = hash_u64(hash, SM64_MODERN_ORACLE_DOMAIN_MARIO);
        hash = hash_u64(hash, 0u);
        hash = hash_u64(hash, MARIO_STATE_ROUTE_RECORD_FIRST + index);
        count++;
    }
...
static SM64ModernStatus trace_write_header(
    ...
    fprintf(stderr,
            "mario_state_route_header mode=%u schema=%u coverage=0x%016" PRIx64 "\n",
...
static SM64ModernStatus trace_write_record(
    ...
    /* Keep the complete Mario state domain, not one selected action field. */
    if (record->domain != SM64_MODERN_ORACLE_DOMAIN_MARIO
        || record->record_kind != SM64_MODERN_ORACLE_RECORD_STATE) {
        return SM64_MODERN_STATUS_OK;
    }

    if (record->record_id < MARIO_STATE_ROUTE_RECORD_FIRST
        || record->record_id > MARIO_STATE_ROUTE_RECORD_LAST
        || record->record_id != MARIO_STATE_ROUTE_RECORD_FIRST + record->sequence) {
        trace->failures++;
        return SM64_MODERN_STATUS_PARITY_DIVERGED;
    }
    const uint32_t expected_value_count =
        record->record_id >= SM64_MODERN_FIELD_MARIO_FACE_ANGLE
            && record->record_id <= SM64_MODERN_FIELD_MARIO_VELOCITY ? 3u : 1u;
    ...
    const uint32_t retained_index = (uint32_t) record->record_id
        - MARIO_STATE_ROUTE_RECORD_FIRST;
    trace->retained_ids[retained_index] = true;
    if (record->record_id == SM64_MODERN_FIELD_MARIO_ACTION) {
        if (trace->action_records == 0u) trace->first_action = record->values[0];
        if (trace->action_records == 1u) trace->second_action = record->values[0];
...
static SM64ModernStatus input_read(
    ...
    snapshot->keyboard_keys[MARIO_STATE_ROUTE_KEY_A / 32u]
        |= UINT32_C(1) << (MARIO_STATE_ROUTE_KEY_A % 32u);
...
static SM64ModernLifecycleConfigV1 make_lifecycle_config(const char *save_directory) {
    ...
    snprintf(config.window_title, sizeof(config.window_title), "%s", "SM64 Modern Mario State Route");
    return config;
}
...
static SM64ModernOracleTraceConfigV1 make_oracle_config(uint64_t timebase_fingerprint) {
    ...
    config.mode = SM64_MODERN_ORACLE_TRACE_RECORD;
    config.build_fingerprint = hash_string("sm64-modern-mario-state-route-build-v1");
    ...
    /* The sidecar retains a route subset; finalize this after the run. */
...
static bool record_route(const char *trace_path, const char *save_directory) {
    ...
    if (ok) {
        /* Initialization emits real schema-4 save/audio records. */
        ...
        if (ok) {
            for (uint32_t index = 0; index < MARIO_STATE_ROUTE_TRACE_STEPS; ++index) {
    ...
    fprintf(stderr,
            "mario_state_route_debug oracle_end=%u result_status=%u actual=%" PRIu64
    ...
    ok = ok && oracle_end == SM64_MODERN_STATUS_OK
        ...
        && trace.records == MARIO_STATE_ROUTE_RECORD_COUNT * MARIO_STATE_ROUTE_TRACE_STEPS
        && trace.action_records == MARIO_STATE_ROUTE_TRACE_STEPS
        && trace.failures == 0u
        && state.errors == 0u
    ...
    if (!ok) {
        fprintf(stderr,
                "mario_state_route_failed errors=%u first_error_status=%u message=%s\n",
                state.errors, state.first_error_status,
                state.first_error_message[0] ? state.first_error_message : "(none)");
        return false;
    }
    printf("c_mario_state_route_recorded shard=0x%016" PRIx64
           " path=%s records=%" PRIu64 " ticks=%" PRIu64
           " first_action=0x%016" PRIx64 " second_action=0x%016" PRIx64
           " coverage=0x%016" PRIx64 "\n",
           MARIO_STATE_ROUTE_SHARD_ID, trace_path, trace.records, trace.last_tick,
           trace.first_action, trace.second_action,
...
int main(int argc, char **argv) {
    if (argc != 3) {
        fprintf(stderr,
                "usage: sm64-modern-mario-state-route-contract TRACE SAVE_DIRECTORY\n");
        return 2;
    }
