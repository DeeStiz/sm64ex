...
/*
 * Phase 85n owns only the canonical object-state route.  The lifecycle and
 * object snapshots are real C owner output; this stream retains one stable
 * active object across the two owner ticks and does not construct records.
 */
#define ROUTE_SHARD_ID UINT64_C(0x862c3d78b60d657c)
#define ROUTE_INPUT_SEED UINT64_C(0x58cc16e4df725004)
#define ROUTE_SAVE_SEED UINT64_C(0x18fda13ffd33fa7d)
#define ROUTE_TRACE_STEPS 2u
#define ROUTE_RECORD_FIRST SM64_MODERN_FIELD_ACTOR_BEHAVIOR
#define ROUTE_RECORD_LAST SM64_MODERN_FIELD_ACTOR_GRAPH_FLAGS
...
static uint64_t coverage_fingerprint(const struct TraceFile *trace) {
    uint64_t hash = ROUTE_FNV_OFFSET;
    uint32_t count = 0;
    for (uint32_t index = 0; index < ROUTE_RECORD_COUNT; ++index) {
        if (!trace->retained_ids[index]) continue;
        hash = hash_u64(hash, SM64_MODERN_ORACLE_DOMAIN_OBJECT);
        hash = hash_u64(hash, 0u);
        hash = hash_u64(hash, ROUTE_RECORD_FIRST + index);
        count++;
    }
...
static SM64ModernStatus trace_write_record(
    ...
    if (trace->target_subject == 0u
        && record->record_id == ROUTE_RECORD_FIRST) {
        trace->target_subject = (uint32_t) record->subject_id;
        fprintf(stderr, "object_state_route_target subject=%u\n", trace->target_subject);
    }
    if (record->subject_id != trace->target_subject) return SM64_MODERN_STATUS_OK;
    ...
    const uint32_t expected_value_count =
        record->record_id == SM64_MODERN_FIELD_ACTOR_POSITION
            || record->record_id == SM64_MODERN_FIELD_ACTOR_VELOCITY
            || record->record_id == SM64_MODERN_FIELD_ACTOR_MOVE_ANGLE ? 3u : 1u;
    ...
    fprintf(stderr, "object_state_route_record tick=%" PRIu64 " sequence=%u subject=%" PRIu64
            " id=%" PRIu64 " values=%u", record->simulation_tick, record->sequence,
            record->subject_id, record->record_id, record->value_count);
    for (uint32_t index = 0; index < record->value_count; ++index) {
        fprintf(stderr, " value[%u]=0x%016" PRIx64, index, record->values[index]);
    }
    if (record->record_id == SM64_MODERN_FIELD_ACTOR_POSITION) {
        uint32_t xBits = (uint32_t) record->values[0];
        uint32_t yBits = (uint32_t) record->values[1];
        uint32_t zBits = (uint32_t) record->values[2];
        float x, y, z;
        memcpy(&x, &xBits, sizeof(x));
        memcpy(&y, &yBits, sizeof(y));
        memcpy(&z, &zBits, sizeof(z));
        fprintf(stderr, " position=(%.9g,%.9g,%.9g)", x, y, z);
    }
    fputc('\n', stderr);
...
static SM64ModernLifecycleConfigV1 make_lifecycle_config(const char *save_directory) {
    ...
    snprintf(config.config_file, sizeof(config.config_file), "%s", "sm64-modern-object-state-route.cfg");
    snprintf(config.window_title, sizeof(config.window_title), "%s", "SM64 Modern Object State Route");
    return config;
}
...
static SM64ModernOracleTraceConfigV1 make_oracle_config(uint64_t timebase_fingerprint) {
    ...
    config.mode = SM64_MODERN_ORACLE_TRACE_RECORD;
    config.build_fingerprint = hash_string("sm64-modern-object-state-route-build-v1");
...
static bool record_route(const char *trace_path, const char *save_directory) {
    ...
    if (ok) {
        ...
        fprintf(stderr, "object_state_route_init status=%u oracle=%u parity=%u\n",
        ...
        if (ok) {
            for (uint32_t index = 0; index < ROUTE_TRACE_STEPS; ++index) {
                ...
                fprintf(stderr, "object_state_route_step index=%u status=%u oracle=%u parity=%u\n",
    ...
    fprintf(stderr, "object_state_route_debug oracle_end=%u result_status=%u actual=%" PRIu64
            " retained=%" PRIu64 " target=%u failures=%u\n", oracle_end, result.status,
            result.actual_records, trace.records, trace.target_subject, trace.failures);
    ok = ok && oracle_end == SM64_MODERN_STATUS_OK
        && result.status == SM64_MODERN_STATUS_OK
        && trace.target_subject > 0u
        ...
        && rewrite_header(&trace, &(SM64ModernOracleTraceConfigV1) {
            ...
            .coverage_fingerprint = coverage_fingerprint(&trace),
        });
    sm64_modern_uninstall_input_api();
    ok = fclose(trace.file) == 0 && ok;
    if (!ok) return false;
    printf("c_object_state_route_recorded shard=0x%016" PRIx64 " subject=%u records=%" PRIu64
           " ticks=2,3 coverage=0x%016" PRIx64 "\n", ROUTE_SHARD_ID,
           trace.target_subject, trace.records, coverage_fingerprint(&trace));
    return true;
}
