 ...
 * the source-owned first TTC clock-hand route.  The C observer is the only
 * producer of the four route receipts; this file never calls the behavior or
 * collision helper and never registers or injects an object.
 */
...
static bool route_record(const SM64ModernOracleTraceRecordV1 *record) {
    ...
    if (record->domain == SM64_MODERN_ORACLE_DOMAIN_SCRIPT
        ...
        && record->subject_id == SM64_MODERN_TTC_ROTATOR_ROUTE_OWNER_ID
        && (record->record_id == SM64_MODERN_TTC_ROTATOR_ROUTE_RECORD_STATE
            || record->record_id == SM64_MODERN_TTC_ROTATOR_ROUTE_RECORD_MOTION)) {
        return true;
    }
    if (record->domain == SM64_MODERN_ORACLE_DOMAIN_COLLISION
        && record->record_kind == SM64_MODERN_ORACLE_RECORD_EVENT
        && record->subject_id == SM64_MODERN_TTC_ROTATOR_ROUTE_OWNER_ID
        && record->record_id == SM64_MODERN_TTC_ROTATOR_ROUTE_RECORD_COLLISION) {
        return true;
    }
    return record->domain == SM64_MODERN_ORACLE_DOMAIN_EFFECT
        && record->record_kind == SM64_MODERN_ORACLE_RECORD_EFFECT
        && record->subject_id == SM64_MODERN_TTC_ROTATOR_ROUTE_OWNER_ID
        && record->record_id == SM64_MODERN_TTC_ROTATOR_ROUTE_RECORD_EFFECT;
}
...
static SM64ModernStatus trace_write_record(
    ...
    if (selected_object && record->value_count == 1u
        && record->record_id == SM64_MODERN_FIELD_ACTOR_BEHAVIOR) {
        if (record->values[0] != SM64_MODERN_TTC_ROTATOR_ROUTE_BEHAVIOR_ID) {
            trace->failures++;
    ...
    if (route_record(record)) {
        if (record->flags != SM64_MODERN_TTC_ROTATOR_ROUTE_FLAGS
            || record->value_count != 8u) {
            trace->failures++;
            return SM64_MODERN_STATUS_PARITY_DIVERGED;
        }
        if (record->domain == SM64_MODERN_ORACLE_DOMAIN_SCRIPT) {
            if (record->record_id == SM64_MODERN_TTC_ROTATOR_ROUTE_RECORD_STATE) {
                trace->state_records++;
            } else {
...
static SM64ModernLifecycleConfigV1 lifecycle_config(const char *save_directory) {
    ...
    snprintf(config.config_file, sizeof(config.config_file), "%s",
             "sm64-modern-ttc-2d-rotator-route.cfg");
    snprintf(config.window_title, sizeof(config.window_title), "%s",
             "SM64 Modern TTC 2D Rotator Route");
    return config;
}
...
static SM64ModernOracleTraceConfigV1 oracle_config(uint64_t timebase_fingerprint) {
    ...
    config.build_fingerprint = hash_string(
        "sm64-modern-ttc-2d-rotator-route-build-v1");
    config.content_fingerprint = hash_string(
        "data/behavior_data.c|src/game/behaviors/ttc_2d_rotator.inc.c|"
        "levels/castle_inside/script.c:111-113|levels/ttc/areas/1/macro.inc.c:40|"
        "bhvTTC2DRotator|MODEL_TTC_CLOCK_HAND|source_observer");
    ...
    config.configuration_fingerprint = hash_string(
        ...
        "painting_nodes=0x21,0x22,0x23;ttc_area=1;clock_hand=first;"
        "shard=0x1af5669b06931d93");
    config.initial_save_fingerprint = hash_string(
        "save=empty-us-slot-0;seed=0xe1463a87da336540");
    config.coverage_fingerprint = 0u;
    return config;
}

static struct Object *find_first_hand(uint32_t *out_count) {
    const BehaviorScript *target = segmented_to_virtual(bhvTTC2DRotator);
...
static bool run_route(const char *trace_path, const char *save_directory) {
    ...
    (void) setenv("SM64_MODERN_AUTOMATED_CASTLE_TTC_ROTATOR", "1", 1);
    ...
    struct Object *first_hand = NULL;
    uint32_t hand_count = 0;
    for (uint32_t step = 0; step < ROUTE_WARMUP_LIMIT; ++step) {
        if (lifecycle.step() != SM64_MODERN_STATUS_OK) break;
        if (gCurrLevelNum == LEVEL_TTC && gCurrentArea != NULL
            && gCurrentArea->index == 1u) {
            first_hand = find_first_hand(&hand_count);
            if (first_hand != NULL && hand_count >= 2u) break;
        }
    }

    if (first_hand == NULL || hand_count < 2u
        || gCurrLevelNum != LEVEL_TTC || gCurrentArea == NULL
        || gCurrentArea->index != 1u) {
        fprintf(stderr,
                "ttc_2d_rotator_route_unreachable level=%d area=%d hands=%u\n",
                gCurrLevelNum,
    ...
    trace.selected_subject = sm64_modern_parity_object_slot(first_hand);
    ...
    sm64_modern_ttc_rotator_route_reset();
    ...
    const SM64ModernTtcRotatorRouteReceiptV1 *receipt =
        sm64_modern_ttc_rotator_route_last_receipt();
    const uint32_t expected_route_records =
        sm64_modern_ttc_rotator_route_matches() * 4u;
    const bool receipt_is_canonical =
        receipt->source_identity == SM64_MODERN_TTC_ROTATOR_ROUTE_SOURCE_ID
        && receipt->owner_identity == SM64_MODERN_TTC_ROTATOR_ROUTE_OWNER_ID
        && receipt->behavior_identity == SM64_MODERN_TTC_ROTATOR_ROUTE_BEHAVIOR_ID
        && receipt->source_subject == trace.selected_subject
        && receipt->source_generation == 1u
        && receipt->source_order == 0u
        && receipt->level == LEVEL_TTC
        && receipt->area == 1u
        && receipt->model == SM64_MODERN_TTC_ROTATOR_ROUTE_MODEL_CLOCK_HAND
        && receipt->behavior_parameter == SM64_MODERN_TTC_ROTATOR_ROUTE_HAND_PARAMETER
        && receipt->flags == SM64_MODERN_TTC_ROTATOR_ROUTE_FLAGS;
    ok = ok
        ...
        && trace.state_records == sm64_modern_ttc_rotator_route_matches()
        && trace.motion_records == sm64_modern_ttc_rotator_route_matches()
        && trace.collision_records == sm64_modern_ttc_rotator_route_matches()
        && trace.effect_records == sm64_modern_ttc_rotator_route_matches()
        ...
        && sm64_modern_ttc_rotator_route_matches() > 0u
        && state.errors == 0u;
    if (!ok) {
        fprintf(stderr,
                "ttc_2d_rotator_route_debug oracle_end=%" PRIu32
                ...
                trace.collision_records,
                trace.effect_records,
                sm64_modern_ttc_rotator_route_invocations(),
                sm64_modern_ttc_rotator_route_matches(),
                sm64_modern_ttc_rotator_route_selected_subject(),
    ...
    printf(
        "ttc_2d_rotator_route_debug oracle_end=0 result_status=0"
        " records=%" PRIu64 " route_records=%u object_behavior=%u"
        " domains=script,object,collision,effect level=ttc area=1\n",
        trace.records,
        expected_route_records,
        trace.behavior_records);
    return true;
...
int main(int argc, char **argv) {
    ...
    if (!run_route(argv[1], argv[2])) {
        /* Missing authored TTC reachability is a blocked precondition, not
         * permission to select a cog, load TTC directly, or emit a fixture. */
        return 77;
    }
    return 0;
}
