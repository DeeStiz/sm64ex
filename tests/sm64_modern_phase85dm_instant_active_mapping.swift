...
private enum Phase85DMError: Error, CustomStringConvertible {
    case invalid(String)
...
private let expectedInstantActiveWarp: UInt64 = 0xf961_678f_e6b6_53ea

private func require(_ condition: @autoclosure () -> Bool, _ message: String) throws {
    guard condition() else { throw Phase85DMError.invalid(message) }
}
...
private func sourceCheck() throws {
    ...
    try require(
        source.contains(
            "OBJECT(/*model*/ MODEL_NONE, /*pos*/ -1024,  512,  -650, "
                + "/*angle*/ 0,    0, 0, /*behParam*/ 0x001E0000, "
                + "/*beh*/ bhvInstantActiveWarp),"
        ),
        "first authored instant-active source entry missing"
    )
    try require(
        source.contains(
            "OBJECT(/*model*/ MODEL_NONE, /*pos*/ -1024,  -50,   717, "
                + "/*angle*/ 0,  180, 0, /*behParam*/ 0x001F0000, "
                + "/*beh*/ bhvInstantActiveWarp),"
        ),
        "subject-40 authored instant-active source entry missing"
    )

    let owner = try String(contentsOfFile: "src/pc/sm64_modern_gameplay_parity.c", encoding: .utf8)
    try require(
        owner.contains("SOURCE_BEHAVIOR_IDENTITY(bhvInstantActiveWarp)"),
        "instant-active semantic owner mapping missing"
    )
    try require(
        fnv("bhvInstantActiveWarp") == expectedInstantActiveWarp,
        "instant-active semantic FNV identity changed"
    )
}
...
private func validateRoute(_ records: [SM64OracleTraceRecord]) throws {
    ...
    let subject40 = record(records, domain: 3, kind: 1, subject: 40, id: 400, tick: 1)
    try require(
        subject40?.values == [expectedInstantActiveWarp],
        "subject-40 instant-active semantic identity mismatch"
    )
    let subject40Yaw = record(records, domain: 3, kind: 1, subject: 40, id: 407, tick: 1)
    try require(
        subject40Yaw?.values == [0, 0xffff_8000, 0],
        "subject-40 instant-active yaw provenance missing"
    )

    let subject41 = record(records, domain: 3, kind: 1, subject: 41, id: 400, tick: 1)
    try require(
        subject41?.values == [expectedInstantActiveWarp],
        "subject-41 instant-active semantic identity mismatch"
    )
    let subject41Yaw = record(records, domain: 3, kind: 1, subject: 41, id: 407, tick: 1)
    try require(
        subject41Yaw?.values == [0, 0, 0],
        "subject-41 instant-active yaw provenance missing"
    )
}
...
private func project(_ input: String, _ output: String) throws {
    ...
    print("phase85dm_swift_projection_written=1 records=\(records.count)")
}
...
private func audit(_ paths: ArraySlice<String>) throws {
    ...
    print(
        "phase85dm_full_parity records=\(values[0].2.count) sequence12_tick=63 "
            + "pcm_records=\(expectedPCMRecords) instant_active_identity=0x"
            + String(expectedInstantActiveWarp, radix: 16)
...
private func reject(_ path: String, _ expected: SM64OracleTraceCodecError...) throws {
    do {
        ...
        throw Phase85DMError.invalid("negative fence accepted \(path)")
...
private func tamper(_ input: String, _ output: String) throws {
    ...
    print("phase85dm_tamper_fence=1")
}
...
private func partial(_ input: String, _ output: String) throws {
    ...
    print("phase85dm_partial_fence=1")
}

@main
struct SM64ModernPhase85DMInstantActiveMapping {
    static func main() {
        do {
            ...
            case "fixture":
                ...
                print("phase85dm_fixture_only_fence=1")
                throw Phase85DMError.invalid("fixture-only evidence rejected")
            case "single":
                try require(arguments.count == 3, "single requires two paths")
                print("phase85dm_single_artifact_fence=1")
                throw Phase85DMError.invalid("single-artifact evidence rejected")
            default:
                throw Phase85DMError.invalid("unknown mode \(mode)")
            }
        } catch {
            FileHandle.standardError.write(
                Data("phase85dm_instant_active_mapping_failed=\(error)\n".utf8)
            )
            exit(1)
        }
    }
