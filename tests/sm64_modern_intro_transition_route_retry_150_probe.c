...
static void platform_error(void *context, SM64ModernStatus status, const char *message) {
    ...
    if (state) {
        state->errors++;
        fprintf(stderr, "intro_transition_retry_150_platform_error status=%u message=%s\n",
...
static SM64ModernOracleTraceConfigV1 make_config(uint64_t timebase_fingerprint) {
    ...
    config.mode = SM64_MODERN_ORACLE_TRACE_RECORD;
    config.build_fingerprint = hash_string("sm64-modern-intro-transition-route-retry-150-build-v1");
    ...
    config.configuration_fingerprint = hash_string(
        ...
        "shard=0x9a0f7b4f7ecf6c41;retry_steps=150");
...
int main(int argc, char **argv) {
    if (argc != 3) {
        fprintf(stderr, "usage: sm64-modern-intro-transition-route-retry-150-probe TRACE SAVE_DIRECTORY\n");
        return 2;
    }
    ...
    if (ok) {
        ...
        fprintf(stderr, "intro_transition_retry_150_init status=%u oracle=%u parity=%u\n",
        ...
        for (uint32_t index = 0; ok && index < ROUTE_STEPS; ++index) {
            ...
            fprintf(stderr, "intro_transition_retry_150_step index=%u status=%u oracle=%u parity=%u\n",
    ...
    fprintf(stderr,
            "intro_transition_retry_150_debug oracle_end=%u result_status=%u actual=%" PRIu64
    ...
    printf("intro_transition_retry_150_route_recorded shard=0x%016" PRIx64
    ...
    puts(trace.transition_records > 0u
         ...
         : "intro_transition_retry_150_route_reachability blocked transition=0");
