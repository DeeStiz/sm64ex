...
@main struct SM64VanishCapSmoke {
    static func main() {
        let rolling = SM64VanishCapBehavior.update(.init(action: 0, timer: 0, faceYaw: 100, forwardVelocity: 2, interacted: false))
        let tangible = SM64VanishCapBehavior.update(.init(action: 0, timer: 21, faceYaw: 0, forwardVelocity: 0, interacted: false))
        precondition(rolling.faceYaw == 356 && !rolling.tangible && tangible.tangible && tangible.opacity == 150)
        ...
        print(String(format: "vanishCapFingerprint=0x%016llx", fingerprint)); print("SM64 Modern vanish cap smoke passed")
    }
}
