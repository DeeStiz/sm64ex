...
private let routeShardID: UInt64 = 0x00ca_b93b_5dd9_4425
private let routeDomain: UInt32 = 11
private let routeRecordKind: UInt32 = 7
private let routeEvent: UInt64 = 0xd1
...
private func readPacket(_ url: URL) throws -> RoutePacket {
    ...
    guard line.count == 1 else { throw SM64OracleTraceCodecError.invalidHeader }
    var fields = [String: String]()
    for field in line[0].split(separator: "|", omittingEmptySubsequences: false) {
    ...
    guard words.count == Int(wordCount) * 2,
          resources.count == Int(wordCount),
          wordCount == 4,
          triangles == 4,
          flags == 3,
          stride(from: 0, to: words.count, by: 2)
              .map({ UInt8(truncatingIfNeeded: words[$0] >> 24) })
              == [0x01, 0x06, 0x06, 0xdf],
          resources[0] == 0x0301_4658,
          resources.dropFirst().allSatisfy({ $0 == 0 }) else {
        throw SM64OracleTraceCodecError.invalidHeader
    }
    var packetHash = routeFNVOffset
    for word in words {
        packetHash = hashU32(packetHash, word)
    }
...
private func coverageFingerprint() -> UInt64 {
    /* Canonical whole-inventory coverage is deferred to ledger admission. */
    return 0
}
...
private func makeRoute(cTrace: URL, packetURL: URL) throws -> (
    ...
    guard c.configuration == routeConfiguration() else {
        throw SM64OracleTraceCodecError.invalidHeader
    }
    guard c.records.count == records.count, c.records == records else {
        throw SM64OracleTraceCodecError.invalidHeader
    }
...
private func writeRoute(cTrace: URL, packetURL: URL, swiftTrace: URL) throws {
    ...
    print(
        "swift_display_list_route_recorded shard=0x\(String(routeShardID, radix: 16))"
            + " records=\(route.records.count) ticks=1,2 words=4 triangles=4"
...
private func audit(cTrace: URL, swiftTrace: URL) throws {
    ...
    print(
        "display_list_pairing_audit admitted=1 c_records=\(c.records.count)"
...
private func tamper(input: URL, output: URL) throws {
    ...
    do {
        ...
        print("display_list_pairing_tamper_rejected=1")
    }
}

@main
struct SM64ModernDisplayListRouteSmoke {
    static func main() {
        do {
            ...
            fputs("display_list_route_swift_error=\(error)\n", stderr)
            exit(1)
        }
    }
}
