...
/*
 * This harness follows the ordinary owner-thread lifecycle.  It only finds
 * the first authored bhvSpindel after Castle Inside SSL painting -> SSL area 1
 * -> the authored area-1 0x14 warp -> SSL area 2.  It never loads SSL directly,
 * calls a behavior/collision helper, injects an object, or synthesizes data.
 */
...
static bool route_record(const SM64ModernOracleTraceRecordV1 *record) {
    if (!record || record->subject_id != SM64_MODERN_SPINDEL_ROUTE_OWNER_ID) {
        return false;
    }
    if (record->domain == SM64_MODERN_ORACLE_DOMAIN_SCRIPT
        && record->record_kind == SM64_MODERN_ORACLE_RECORD_EVENT
        && (record->record_id == SM64_MODERN_SPINDEL_ROUTE_RECORD_STATE
            || record->record_id == SM64_MODERN_SPINDEL_ROUTE_RECORD_MOTION)) {
        return true;
    }
    if (record->domain == SM64_MODERN_ORACLE_DOMAIN_COLLISION
        && record->record_kind == SM64_MODERN_ORACLE_RECORD_EVENT
        && record->record_id == SM64_MODERN_SPINDEL_ROUTE_RECORD_COLLISION) {
        return true;
    }
    return record->domain == SM64_MODERN_ORACLE_DOMAIN_EFFECT
        && record->record_kind == SM64_MODERN_ORACLE_RECORD_EFFECT
        && record->record_id == SM64_MODERN_SPINDEL_ROUTE_RECORD_EFFECT;
}
...
static SM64ModernStatus trace_write_record(
    ...
    if (selected_object && record->record_id == SM64_MODERN_FIELD_ACTOR_BEHAVIOR) {
        if (record->value_count != 1u
            || record->values[0] != SM64_MODERN_SPINDEL_ROUTE_BEHAVIOR_ID) {
            trace->failures++;
    ...
    if (route_record(record)) {
        if (record->flags != SM64_MODERN_SPINDEL_ROUTE_FLAGS
            || record->value_count != SM64_MODERN_ORACLE_TRACE_VALUE_CAPACITY) {
            trace->failures++;
            return SM64_MODERN_STATUS_PARITY_DIVERGED;
        }
        if (record->domain == SM64_MODERN_ORACLE_DOMAIN_SCRIPT) {
            if (record->record_id == SM64_MODERN_SPINDEL_ROUTE_RECORD_STATE) {
                trace->state_records++;
            } else {
...
static SM64ModernOracleTraceConfigV1 oracle_config(uint64_t timebase_fingerprint) {
    ...
    config.mode = SM64_MODERN_ORACLE_TRACE_RECORD;
    config.build_fingerprint = hash_string("sm64-modern-spindel-route-build-v1");
    config.content_fingerprint = hash_string(
        "data/behavior_data.c:4380-4388|"
        "src/game/behaviors/spindel.inc.c|"
        "levels/castle_inside/script.c:173-175|"
        "levels/ssl/script.c:41-56,93-100,115-131|"
        "bhvSpindel|MODEL_SSL_SPINDEL|ssl_seg7_collision_spindel|source_observer");
    ...
    config.configuration_fingerprint = hash_string(
        ...
        "castle_ssl_painting_nodes=0x0f,0x10,0x11;ssl_area1_warp=0x14;"
        "ssl_area=2;script_func_local_4_ordinal=5;spindel=first;"
        "position=-2458,2109,-1430;yaw=0;parameter=0;"
        "shard=0xdc93743116807bec");
    config.initial_save_fingerprint = hash_string(
        "save=empty-us-slot-0;seed=0x4b4378861cfabced");
    config.coverage_fingerprint = 0u;
    return config;
}

static struct Object *find_first_spindel(uint32_t *out_count) {
    const BehaviorScript *target = segmented_to_virtual(bhvSpindel);
...
static bool run_route(const char *trace_path, const char *save_directory) {
    ...
    /* The lifecycle starts in Castle; authored warps remain the only route. */
    ...
    struct Object *first_spindel = NULL;
    uint32_t spindel_count = 0;
    for (uint32_t step = 0; step < ROUTE_WARMUP_LIMIT; ++step) {
        if (lifecycle.step() != SM64_MODERN_STATUS_OK) break;
        if (gCurrLevelNum == LEVEL_SSL && gCurrentArea != NULL
            && gCurrentArea->index == SM64_MODERN_SPINDEL_ROUTE_AREA) {
            first_spindel = find_first_spindel(&spindel_count);
            if (first_spindel != NULL && spindel_count == 1u) break;
        }
    }

    if (first_spindel == NULL || spindel_count != 1u
        || gCurrLevelNum != LEVEL_SSL || gCurrentArea == NULL
        || gCurrentArea->index != SM64_MODERN_SPINDEL_ROUTE_AREA) {
        fprintf(stderr,
                "spindel_route_unreachable level=%d area=%d spindels=%u\n",
                gCurrLevelNum,
    ...
    trace.selected_subject = sm64_modern_parity_object_slot(first_spindel);
    ...
    sm64_modern_spindel_route_reset();
    ...
    const SM64ModernSpindelRouteReceiptV1 *receipt =
        sm64_modern_spindel_route_last_receipt();
    const uint32_t expected_route_records =
        sm64_modern_spindel_route_matches() * 4u;
    const bool receipt_is_canonical =
        receipt->source_identity == SM64_MODERN_SPINDEL_ROUTE_SOURCE_ID
        && receipt->owner_identity == SM64_MODERN_SPINDEL_ROUTE_OWNER_ID
        && receipt->behavior_identity == SM64_MODERN_SPINDEL_ROUTE_BEHAVIOR_ID
        && receipt->source_subject == trace.selected_subject
        && receipt->source_generation == 1u
        && receipt->source_order == SM64_MODERN_SPINDEL_ROUTE_SOURCE_ORDER
        && receipt->level == LEVEL_SSL
        && receipt->area == SM64_MODERN_SPINDEL_ROUTE_AREA
        && receipt->model == SM64_MODERN_SPINDEL_ROUTE_MODEL
        && receipt->behavior_parameter
            == SM64_MODERN_SPINDEL_ROUTE_BEHAVIOR_PARAMETER
        && receipt->face_yaw == SM64_MODERN_SPINDEL_ROUTE_SOURCE_FACE_YAW
        && receipt->collision_model_identity
            == SM64_MODERN_SPINDEL_ROUTE_COLLISION_IDENTITY
        && receipt->collision_model_loaded == 1u
        && receipt->flags == SM64_MODERN_SPINDEL_ROUTE_FLAGS;
    ok = ok
        ...
        && trace.state_records == sm64_modern_spindel_route_matches()
        && trace.motion_records == sm64_modern_spindel_route_matches()
        && trace.collision_records == sm64_modern_spindel_route_matches()
        && trace.effect_records == sm64_modern_spindel_route_matches()
        ...
        && sm64_modern_spindel_route_matches() > 0u
        && state.errors == 0u;
    if (!ok) {
        fprintf(stderr,
                "spindel_route_debug oracle_end=%" PRIu32
                ...
                trace.collision_records,
                trace.effect_records,
                sm64_modern_spindel_route_invocations(),
                sm64_modern_spindel_route_matches(),
                sm64_modern_spindel_route_selected_subject(),
    ...
    printf(
        "spindel_route_debug oracle_end=0 result_status=0"
        " records=%" PRIu64 " route_records=%u object_behavior=%u"
        " domains=script,object,collision,effect level=ssl area=2\n",
        trace.records,
        expected_route_records,
        trace.behavior_records);
    return true;
...
int main(int argc, char **argv) {
    ...
    if (!run_route(argv[1], argv[2])) {
        /* Missing authored Castle -> SSL -> area-2 reachability is a blocked
         * precondition, not permission to load SSL or synthesize a receipt. */
        return 77;
    }
    return 0;
}
