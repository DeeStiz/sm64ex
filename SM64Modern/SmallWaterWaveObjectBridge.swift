import Foundation

struct SM64SmallWaterWaveObjectEffectRecord: Equatable, Sendable {
    let objectID: SM64ObjectID
    let output: SM64SmallWaterWaveOutput
    let spawnedSplash: SM64ObjectID?
}

final class SM64SmallWaterWaveObjectBridge {
    static let defaultBehaviorIdentity: UInt64 = 0x6268_765F_737777
    static let objectWaterSplashBehaviorIdentity: UInt64 = SM64WaterSplashObjectBridge.objectBehaviorIdentity
    static let defaultModel: UInt32 = 0

    private let waterSplashBridge: SM64WaterSplashObjectBridge?

    private struct State {
        let waterLevel: Float
        var angleX: Int32
        var angleZ: Int32
        let angleVelocityX: Int32
        let angleVelocityZ: Int32
    }

    private var states: [SM64ObjectID: State] = [:]
    private(set) var effectLog: [SM64SmallWaterWaveObjectEffectRecord] = []

    init(waterSplashBridge: SM64WaterSplashObjectBridge? = nil) {
        self.waterSplashBridge = waterSplashBridge
    }

    var registeredIDs: [SM64ObjectID] {
        states.keys.sorted {
            $0.slot == $1.slot ? $0.generation < $1.generation : $0.slot < $1.slot
        }
    }

    func beginExternalTick() { effectLog.removeAll(keepingCapacity: true) }

    @discardableResult
    func spawnWave(
        in engineState: SM64SwiftEngineState,
        position: SM64ObjectVector3 = .zero,
        waterLevel: Float = 0,
        angleX: Int32 = 0,
        angleZ: Int32 = 0,
        angleVelocityX: Int32 = 0x400,
        angleVelocityZ: Int32 = 0x400
    ) throws -> SM64ObjectID {
        let id = try engineState.spawnObject(
            in: .unimportant,
            model: Self.defaultModel,
            behaviorIdentity: Self.defaultBehaviorIdentity
        )
        guard attach(
            id,
            position: position,
            waterLevel: waterLevel,
            angleX: angleX,
            angleZ: angleZ,
            angleVelocityX: angleVelocityX,
            angleVelocityZ: angleVelocityZ,
            in: engineState.objects
        ) else {
            _ = engineState.objects.despawn(id)
            preconditionFailure("newly spawned small water wave could not attach")
        }
        return id
    }

    @discardableResult
    func attach(
        _ id: SM64ObjectID,
        position: SM64ObjectVector3,
        waterLevel: Float,
        angleX: Int32,
        angleZ: Int32,
        angleVelocityX: Int32,
        angleVelocityZ: Int32,
        in pool: SM64ObjectPool
    ) -> Bool {
        guard pool.record(for: id) != nil else { return false }
        states[id] = State(
            waterLevel: waterLevel,
            angleX: angleX,
            angleZ: angleZ,
            angleVelocityX: angleVelocityX,
            angleVelocityZ: angleVelocityZ
        )
        return pool.mutate(id) { record in
            record.position = position
            record.homePosition = position
            record.scale = .one
            record.objectFlags |= SM64ObjectScheduler.objectFlagUpdateGfxPositionAndAngle
                | SM64ObjectScheduler.objectFlagBuildTransform
        }
    }

    @discardableResult
    func updateInline(
        _ id: SM64ObjectID,
        state engineState: SM64SwiftEngineState
    ) -> SM64SmallWaterWaveObjectEffectRecord? {
        guard var state = states[id], let record = engineState.objects.record(for: id) else {
            return nil
        }
        let output = SM64SmallWaterWaveBehavior.update(
            SM64SmallWaterWaveInput(
                position: record.position,
                waterLevel: state.waterLevel,
                angleX: state.angleX,
                angleZ: state.angleZ,
                angleVelocityX: state.angleVelocityX,
                angleVelocityZ: state.angleVelocityZ,
                timer: record.timer,
                interacted: record.interactionStatus != 0
            )
        )
        state.angleX = output.angleX
        state.angleZ = output.angleZ
        states[id] = state
        var splash: SM64ObjectID?
        if output.spawnSplash {
            splash = try? engineState.spawnObject(
                in: .unimportant,
                model: 0,
                behaviorIdentity: Self.objectWaterSplashBehaviorIdentity,
                parent: id
            )
            if let splash {
                _ = waterSplashBridge?.attachObjectSplash(
                    splash,
                    position: output.position,
                    in: engineState.objects
                )
            }
        }
        _ = engineState.objects.mutate(id) { next in
            next.position = output.position
            next.scale.x = output.scaleX
            next.scale.z = output.scaleZ
            next.timer = output.timer
            next.animationState = output.timer
            next.interactionStatus = 0
            next.objectFlags |= SM64ObjectScheduler.objectFlagUpdateGfxPositionAndAngle
                | SM64ObjectScheduler.objectFlagBuildTransform
            if output.shouldDelete { next.activeFlags = 0 }
            else if output.shouldDeactivate { next.activeFlags = 0 }
        }
        let effect = SM64SmallWaterWaveObjectEffectRecord(
            objectID: id,
            output: output,
            spawnedSplash: splash
        )
        effectLog.append(effect)
        return effect
    }

    func remove(_ id: SM64ObjectID) { states.removeValue(forKey: id) }
    func pruneExternal(unloaded: [SM64ObjectID], pool: SM64ObjectPool) {
        for id in unloaded { remove(id) }
        for id in registeredIDs where pool.record(for: id) == nil { remove(id) }
    }
}
