import Foundation
struct SM64WingCapObjectEffectRecord: Equatable, Sendable { let objectID: SM64ObjectID; let output: SM64WingCapOutput }
final class SM64WingCapObjectBridge {
    static let defaultBehaviorIdentity: UInt64 = 0x6268_765F_77636170
    private var registered: Set<SM64ObjectID> = []
    private(set) var effectLog: [SM64WingCapObjectEffectRecord] = []
    ...
    func spawn(in engineState: SM64SwiftEngineState, position: SM64ObjectVector3 = .zero, forwardVelocity: Float = 0) throws -> SM64ObjectID {
        ...
        guard engineState.objects.mutate(id, { record in record.position = position; record.forwardVelocity = forwardVelocity; record.gravity = 1.2; record.friction = 0.999; record.buoyancy = 0.9; record.opacity = 255; record.hitboxRadius = 80; record.hitboxHeight = 80; record.objectFlags |= SM64ObjectScheduler.objectFlagUpdateGfxPositionAndAngle | SM64ObjectScheduler.objectFlagBuildTransform }) else { _ = engineState.objects.despawn(id); preconditionFailure("newly spawned wing cap could not attach") }
    ...
    func updateInline(_ id: SM64ObjectID, state engineState: SM64SwiftEngineState) -> Bool {
        ...
        let output = SM64WingCapBehavior.update(.init(action: record.action, timer: record.timer, faceYaw: record.faceAngles.yaw, forwardVelocity: record.forwardVelocity, interacted: record.interactionStatus != 0))
