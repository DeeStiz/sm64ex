...
BUILD_ROOT="$PROJECT_ROOT/build/sm64-modern-small-penguin-object-bridge"
mkdir -p "$BUILD_ROOT"
...
SWIFT_OUTPUT="$($BUILD_ROOT/sm64-modern-small-penguin-object-bridge-smoke)"
printf '%s\n' "$SWIFT_OUTPUT"

xcrun clang -std=c11 -ffp-contract=off \
  "$PROJECT_ROOT/tests/sm64_modern_small_penguin_object_bridge_contract.c" \
  -o "$BUILD_ROOT/sm64-modern-small-penguin-object-bridge-contract"
C_OUTPUT="$($BUILD_ROOT/sm64-modern-small-penguin-object-bridge-contract)"
printf '%s\n' "$C_OUTPUT"

SWIFT_FINGERPRINT="$(printf '%s\n' "$SWIFT_OUTPUT" | sed -n 's/^smallPenguinObjectBridgeFingerprint=//p')"
C_FINGERPRINT="$(printf '%s\n' "$C_OUTPUT" | sed -n 's/^smallPenguinObjectBridgeFingerprint=//p')"
[[ -n "$SWIFT_FINGERPRINT" && "$SWIFT_FINGERPRINT" == "$C_FINGERPRINT" ]] || {
  echo "Swift/C small penguin object bridge fingerprint mismatch: Swift=$SWIFT_FINGERPRINT C=$C_FINGERPRINT" >&2
  exit 1
}
printf '%s\n' "SM64 Modern small penguin object bridge C contract matched"
