...
BUILD_ROOT="$PROJECT_ROOT/build/sm64-modern-npc-menu"
...
xcrun swiftc \
  ...
  "$PROJECT_ROOT/tests/sm64_modern_npc_menu_smoke.swift" \
  -o "$BUILD_ROOT/smoke"
...
xcrun clang -std=c11 \
  "$PROJECT_ROOT/tests/sm64_modern_npc_menu_contract.c" \
  -o "$BUILD_ROOT/contract"
c_output="$($BUILD_ROOT/contract)"
printf '%s\n' "$c_output"

swift_fingerprint="$(printf '%s\n' "$swift_output" | sed -n 's/^npcMenuFingerprint=//p')"
c_fingerprint="$(printf '%s\n' "$c_output" | sed -n 's/^npcMenuFingerprint=//p')"
[[ -n "$swift_fingerprint" && "$swift_fingerprint" == "$c_fingerprint" ]] || {
  echo "Swift/C NPC menu contract mismatch: Swift=$swift_fingerprint C=$c_fingerprint" >&2
  exit 1
}
printf '%s\n' 'Swift/C NPC menu contract matched'
