...
# Phase 85cy is a disjoint full-trace rerun for Castle Inside area-1 subject
# 32, the authored 90-degree bhvAirborneStarCollectWarp. All build, trace,
...
BUILD_ROOT="${SM64_PHASE85CY_BUILD_ROOT:-$PROJECT_ROOT/build/sm64-modern-phase85cy-airborne-star-mapping}"
...
SWIFT_OUTPUT="$TOOL_ROOT/airborne-star-mapping-swift"
...
xcrun swiftc -parse-as-library -swift-version 6 -Xfrontend -strict-concurrency=complete \
    ...
    "$PROJECT_ROOT/tests/sm64_modern_phase85cy_airborne_star_mapping.swift" \
    -o "$SWIFT_OUTPUT"
...
if "$SWIFT_OUTPUT" fixture "$C_TRACE" "$FIXTURE_MARKER"; then
    echo 'phase85cy fixture-only fence unexpectedly accepted' >&2
    exit 1
fi
if "$SWIFT_OUTPUT" single "$C_TRACE" "$C_TRACE"; then
    echo 'phase85cy single-artifact fence unexpectedly accepted' >&2
    exit 1
fi
...
if "$SWIFT_OUTPUT" audit "$C_TRACE" "$SWIFT_TRACE" "$ASAN_TRACE" \
    ...
    audit_status=$?
    echo 'phase85cy fail-closed: next full-trace mismatch remains unmapped' >&2
fi

git -c core.fsmonitor=false diff --check -- \
    src/pc/sm64_modern_gameplay_parity.c \
    script/test_phase85cy_airborne_star_mapping.sh \
    tests/sm64_modern_phase85cy_airborne_star_mapping.swift \
    .porting/porting-handoff-full-swift-twin-phase85cy-airborne-star-mapping.md

printf '%s\n' \
    'SM64 Modern Phase 85cy airborne-star mapping matrix completed' \
    'native_c_lifecycle=1 castle_area2=1 sequence12_tick=63 records=476365' \
    'subject32_source=bhvAirborneStarCollectWarp yaw90=1 semantic_identity=0xa85510394290b349' \
