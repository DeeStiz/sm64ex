...
BUILD_ROOT="$PROJECT_ROOT/build/sm64-modern-bully-reward-presentation"
mkdir -p "$BUILD_ROOT"
...
xcrun swiftc \
  ...
  -o "$BUILD_ROOT/sm64-modern-bully-reward-presentation-smoke"

SWIFT_OUTPUT="$($BUILD_ROOT/sm64-modern-bully-reward-presentation-smoke)"
printf '%s\n' "$SWIFT_OUTPUT"

xcrun clang -std=c11 \
  "$PROJECT_ROOT/tests/sm64_modern_bully_reward_presentation_contract.c" \
  -o "$BUILD_ROOT/sm64-modern-bully-reward-presentation-contract"
C_OUTPUT="$($BUILD_ROOT/sm64-modern-bully-reward-presentation-contract)"
printf '%s\n' "$C_OUTPUT"

SWIFT_FINGERPRINT="$(printf '%s\n' "$SWIFT_OUTPUT" | sed -n 's/^bullyRewardPresentationFingerprint=//p')"
C_FINGERPRINT="$(printf '%s\n' "$C_OUTPUT" | sed -n 's/^bullyRewardPresentationFingerprint=//p')"
[[ -n "$SWIFT_FINGERPRINT" && "$SWIFT_FINGERPRINT" == "$C_FINGERPRINT" ]] || {
  echo "Swift/C Bully reward fingerprint mismatch: Swift=$SWIFT_FINGERPRINT C=$C_FINGERPRINT" >&2
  exit 1
}
printf '%s\n' "SM64 Modern Bully reward presentation C contract matched"
