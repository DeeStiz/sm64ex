...
BUILD_ROOT="$PROJECT_ROOT/build/sm64-modern-whomp-object-bridge-smoke"
mkdir -p "$BUILD_ROOT"
...
xcrun swiftc \
  ...
  "$PROJECT_ROOT/tests/sm64_modern_whomp_object_bridge_smoke.swift" \
  -o "$BUILD_ROOT/sm64-modern-whomp-object-bridge-smoke"

SWIFT_OUTPUT="$($BUILD_ROOT/sm64-modern-whomp-object-bridge-smoke)"
printf '%s\n' "$SWIFT_OUTPUT"

xcrun clang -std=c11 \
  "$PROJECT_ROOT/tests/sm64_modern_whomp_object_bridge_contract.c" \
  -o "$BUILD_ROOT/sm64-modern-whomp-object-bridge-contract"
C_OUTPUT="$($BUILD_ROOT/sm64-modern-whomp-object-bridge-contract)"
printf '%s\n' "$C_OUTPUT"

SWIFT_FINGERPRINT="$(printf '%s\n' "$SWIFT_OUTPUT" | sed -n 's/^whompObjectBridgeFingerprint=//p')"
C_FINGERPRINT="$(printf '%s\n' "$C_OUTPUT" | sed -n 's/^whompObjectBridgeFingerprint=//p')"
[[ -n "$SWIFT_FINGERPRINT" && "$SWIFT_FINGERPRINT" == "$C_FINGERPRINT" ]] || {
  echo "Swift/C Whomp object bridge fingerprint mismatch: Swift=$SWIFT_FINGERPRINT C=$C_FINGERPRINT" >&2
  exit 1
}
printf '%s\n' "SM64 Modern Whomp object bridge C contract matched"
