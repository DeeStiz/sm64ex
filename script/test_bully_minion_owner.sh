...
BUILD_ROOT="$PROJECT_ROOT/build/sm64-modern-bully-minion-owner"
mkdir -p "$BUILD_ROOT"
...
xcrun swiftc \
  ...
  "$PROJECT_ROOT/tests/sm64_modern_bully_minion_owner_smoke.swift" \
  -o "$BUILD_ROOT/sm64-modern-bully-minion-owner-smoke"

SWIFT_OUTPUT="$($BUILD_ROOT/sm64-modern-bully-minion-owner-smoke)"
printf '%s\n' "$SWIFT_OUTPUT"

xcrun clang -std=c11 \
  "$PROJECT_ROOT/tests/sm64_modern_bully_minion_owner_contract.c" \
  -o "$BUILD_ROOT/sm64-modern-bully-minion-owner-contract"
C_OUTPUT="$($BUILD_ROOT/sm64-modern-bully-minion-owner-contract)"
printf '%s\n' "$C_OUTPUT"

SWIFT_FINGERPRINT="$(printf '%s\n' "$SWIFT_OUTPUT" | sed -n 's/^bullyMinionOwnerFingerprint=//p')"
C_FINGERPRINT="$(printf '%s\n' "$C_OUTPUT" | sed -n 's/^bullyMinionOwnerFingerprint=//p')"
[[ -n "$SWIFT_FINGERPRINT" && "$SWIFT_FINGERPRINT" == "$C_FINGERPRINT" ]] || {
  echo "Swift/C Bully minion fingerprint mismatch: Swift=$SWIFT_FINGERPRINT C=$C_FINGERPRINT" >&2
  exit 1
}
printf '%s\n' "SM64 Modern Bully minion owner C contract matched"
