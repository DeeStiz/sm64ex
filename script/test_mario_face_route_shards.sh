...
BUILD_ROOT="$PROJECT_ROOT/build/sm64-modern-mario-face-route-shards"
...
xcrun swiftc \
  ...
  "$PROJECT_ROOT/tests/sm64_modern_mario_face_route_shards_smoke.swift" \
  -o "$BUILD_ROOT/mario-face-route-shards-smoke"

xcrun clang -std=c11 -Wall -Wextra -Werror \
  -I"$PROJECT_ROOT" -I"$PROJECT_ROOT/include" -I"$PROJECT_ROOT/src" \
  "$PROJECT_ROOT/src/pc/sm64_modern_oracle_trace.c" \
  "$PROJECT_ROOT/tests/sm64_modern_mario_face_route_shards_contract.c" \
  -o "$BUILD_ROOT/mario-face-route-shards-contract"

TRACE="$BUILD_ROOT/mario-face-route-shards.trace"
SWIFT_OUTPUT="$($BUILD_ROOT/mario-face-route-shards-smoke "$TRACE")"
C_OUTPUT="$($BUILD_ROOT/mario-face-route-shards-contract "$TRACE")"
TAMPER_OUTPUT="$($BUILD_ROOT/mario-face-route-shards-contract "$TRACE" --tamper)"
printf '%s\n' "$SWIFT_OUTPUT" "$C_OUTPUT" "$TAMPER_OUTPUT"

for LABEL in marioFaceRouteShardCount marioFaceRouteShardRecords marioFaceRouteShardFingerprint; do
  SWIFT_VALUE="$(printf '%s\n' "$SWIFT_OUTPUT" | sed -n "s/^${LABEL}=//p")"
  C_VALUE="$(printf '%s\n' "$C_OUTPUT" | sed -n "s/^${LABEL}=//p")"
  [[ -n "$SWIFT_VALUE" && "$SWIFT_VALUE" == "$C_VALUE" ]] || {
    echo "Mario-face route shard mismatch for $LABEL: Swift=$SWIFT_VALUE C=$C_VALUE" >&2
    exit 1
  }
done
