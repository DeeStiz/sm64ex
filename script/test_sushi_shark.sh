...
BUILD_ROOT="$PROJECT_ROOT/build/sm64-modern-sushi-shark"
...
xcrun swiftc -parse-as-library -swift-version 6 -Xfrontend -strict-concurrency=complete \
  ...
  "$PROJECT_ROOT/tests/sm64_modern_sushi_shark_smoke.swift" \
  -o "$BUILD_ROOT/smoke"
swift_output="$($BUILD_ROOT/smoke)"
printf '%s\n' "$swift_output"

xcrun clang -std=c11 "$PROJECT_ROOT/tests/sm64_modern_sushi_shark_contract.c" -o "$BUILD_ROOT/contract"
c_output="$($BUILD_ROOT/contract)"
printf '%s\n' "$c_output"

swift_fingerprint="$(printf '%s\n' "$swift_output" | sed -n 's/^sushiSharkFingerprint=//p')"
c_fingerprint="$(printf '%s\n' "$c_output" | sed -n 's/^sushiSharkFingerprint=//p')"
[[ -n "$swift_fingerprint" && "$swift_fingerprint" == "$c_fingerprint" ]] || {
  echo "Sushi shark mismatch Swift=$swift_fingerprint C=$c_fingerprint" >&2
  exit 1
}
printf '%s\n' 'Swift/C Sushi shark contract matched'
