...
require_fixed 'MTL_CAPTURE_ENABLED' "$COMPILER"
require_fixed 'captureArchiveBypassed' "$COMPILER"
require_fixed 'metal4_archive_capture_bypass' "$COMPILER"
require_fixed 'compiler_fallback=enabled' "$COMPILER"
require_fixed 'descriptor.configuration = MTL4PipelineDataSetSerializerConfigurationCaptureBinaries;' "$BRIDGE"
require_fixed 'SM64ModernMakeRenderPipelineStateAsync' "$COMPILER"
require_fixed 'SM64ModernLookupRenderPipelineState' "$COMPILER"
...
require_fixed 'metal4_cache_diagnostic' "$COMPILER"
require_fixed 'metal4_archive_flush_result' "$COMPILER"
require_fixed 'metal4_archive_flush_skipped' "$COMPILER"
require_fixed 'metal_validation_profile' "$COMPILER"
require_fixed 'waitUntilReady(for keys: [MetalShaderKey]) throws' "$COMPILER"
require_fixed 'compileQueue.sync {}' "$COMPILER"
require_fixed 'waitForPreparedPipelines() throws' "$COMPILER"
require_fixed 'try shaderCompiler.waitForPreparedPipelines()' "$RENDERER"
...
require_fixed 'metal_presentation_diagnostic' "$RENDERER"
require_fixed 'host_compositor_evidence=' "$RENDERER"
