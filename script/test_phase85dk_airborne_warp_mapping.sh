...
# Phase 85dk is a disjoint full-trace rerun for Castle Inside area-1 subject
# 39, the authored 180-degree bhvAirborneWarp. All build, trace, PCM, receipt,
# projection, and negative-fence artifacts remain phase-local. This harness
# must not mutate canonical manifests, ledgers, reports, or shared docs.
bash -n "$0"

PROJECT_ROOT="$(cd "$(dirname "$0")/.." && pwd)"
BUILD_ROOT="${SM64_PHASE85DK_BUILD_ROOT:-$PROJECT_ROOT/build/sm64-modern-phase85dk-airborne-warp-mapping}"
...
SWIFT_OUTPUT="$TOOL_ROOT/airborne-warp-mapping-swift"
...
xcrun swiftc -parse-as-library -swift-version 6 -Xfrontend -strict-concurrency=complete \
    ...
    "$PROJECT_ROOT/tests/sm64_modern_phase85dk_airborne_warp_mapping.swift" \
    -o "$SWIFT_OUTPUT"
...
if "$SWIFT_OUTPUT" fixture "$C_TRACE" "$FIXTURE_MARKER"; then
    echo 'phase85dk fixture-only fence unexpectedly accepted' >&2
    exit 1
fi
if "$SWIFT_OUTPUT" single "$C_TRACE" "$C_TRACE"; then
    echo 'phase85dk single-artifact fence unexpectedly accepted' >&2
    exit 1
fi
...
if "$SWIFT_OUTPUT" audit "$C_TRACE" "$SWIFT_TRACE" "$ASAN_TRACE" \
    ...
    audit_status=$?
    echo 'phase85dk fail-closed: next full-trace mismatch remains unmapped' >&2
fi

git -c core.fsmonitor=false diff --check -- \
    src/pc/sm64_modern_gameplay_parity.c \
    script/test_phase85dk_airborne_warp_mapping.sh \
    tests/sm64_modern_phase85dk_airborne_warp_mapping.swift \
    .porting/porting-handoff-full-swift-twin-phase85dk-airborne-warp-mapping.md

printf '%s\n' \
    'SM64 Modern Phase 85dk airborne-warp mapping matrix completed' \
    'native_c_lifecycle=1 castle_area2=1 sequence12_tick=63 records=476365' \
    'subject39_source=bhvAirborneWarp yaw180=1 semantic_identity=0x3e6af9ed47c59929' \
