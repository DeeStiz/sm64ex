...
# Phase 85cs is a disjoint rerun harness for the authored Castle high-score
# audio route after the bhvPaintingStarCollectWarp source mapping.  All build,
# trace, PCM, receipt, and negative-fence artifacts remain phase-local.  This
# harness must not mutate canonical manifests, ledgers, reports, admission
# tools, or shared documentation.
bash -n "$0"

PROJECT_ROOT="$(cd "$(dirname "$0")/.." && pwd)"
BUILD_ROOT="${SM64_PHASE85CS_BUILD_ROOT:-$PROJECT_ROOT/build/sm64-modern-phase85cs-audio-parity-subject27}"
...
SWIFT_OUTPUT="$TOOL_ROOT/audio-parity-subject27-swift"
...
xcrun swiftc -parse-as-library -swift-version 6 -Xfrontend -strict-concurrency=complete \
    ...
    "$PROJECT_ROOT/tests/sm64_modern_phase85cs_audio_parity_subject27.swift" \
    -o "$SWIFT_OUTPUT"
...
if ! "$SWIFT_OUTPUT" audit "$C_TRACE" "$SWIFT_TRACE" "$ASAN_TRACE" "$RELEASE_TRACE" "$RERUN_TRACE"; then
    echo 'phase85cs fail-closed: cross-build full-trace parity did not pass' >&2
    exit 1
fi
...
if "$SWIFT_OUTPUT" fixture "$C_TRACE" "$FIXTURE_MARKER"; then
    echo 'phase85cs fixture-only fence unexpectedly accepted' >&2
    exit 1
fi
if "$SWIFT_OUTPUT" single "$C_TRACE" "$C_TRACE"; then
    echo 'phase85cs single-artifact fence unexpectedly accepted' >&2
    exit 1
fi
...
git -c core.fsmonitor=false diff --check -- \
    script/test_phase85cs_audio_parity_subject27.sh \
    tests/sm64_modern_phase85cs_audio_parity_subject27.swift
printf '%s\n' \
    'SM64 Modern Phase 85cs audio parity subject27 passed exact_pair=1' \
    'native_c_lifecycle=1 castle_area2=1 sequence12_tick=63 records=476365' \
    'semantic_mappings=sign_onecoin_floortrap_boo_painting_star_subject27=1' \
    'c_swift_asan_release_rerun_byte_match=1 pcm_match=1 receipts_match=1 tamper_rejected=1 partial_rejected=1 fixture_only_rejected=1 single_artifact_rejected=1' \
