...
BUILD_ROOT="$PROJECT_ROOT/build/sm64-modern-explosion"
mkdir -p "$BUILD_ROOT"
...
SWIFT_OUTPUT="$($BUILD_ROOT/sm64-modern-explosion-smoke)"
printf '%s\n' "$SWIFT_OUTPUT"

xcrun clang -std=c11 \
  "$PROJECT_ROOT/tests/sm64_modern_explosion_contract.c" \
  -o "$BUILD_ROOT/sm64-modern-explosion-contract"
C_OUTPUT="$($BUILD_ROOT/sm64-modern-explosion-contract)"
printf '%s\n' "$C_OUTPUT"

SWIFT_FINGERPRINT="$(printf '%s\n' "$SWIFT_OUTPUT" | sed -n 's/^explosionFingerprint=//p')"
C_FINGERPRINT="$(printf '%s\n' "$C_OUTPUT" | sed -n 's/^explosionFingerprint=//p')"
[[ -n "$SWIFT_FINGERPRINT" && "$SWIFT_FINGERPRINT" == "$C_FINGERPRINT" ]] || {
  echo "Swift/C explosion fingerprint mismatch: Swift=$SWIFT_FINGERPRINT C=$C_FINGERPRINT" >&2
  exit 1
}
printf '%s\n' "Swift/C explosion contract matched"
