...
# Phase 85dg is a disjoint full-trace rerun for Castle Inside area-1 subject
# 37, the authored 180-degree bhvHardAirKnockBackWarp. All build, trace,
...
BUILD_ROOT="${SM64_PHASE85DG_BUILD_ROOT:-$PROJECT_ROOT/build/sm64-modern-phase85dg-hard-air-mapping}"
...
SWIFT_OUTPUT="$TOOL_ROOT/hard-air-mapping-swift"
...
xcrun swiftc -parse-as-library -swift-version 6 -Xfrontend -strict-concurrency=complete \
    ...
    "$PROJECT_ROOT/tests/sm64_modern_phase85dg_hard_air_mapping.swift" \
    -o "$SWIFT_OUTPUT"
...
if "$SWIFT_OUTPUT" fixture "$C_TRACE" "$FIXTURE_MARKER"; then
    echo 'phase85dg fixture-only fence unexpectedly accepted' >&2
    exit 1
fi
if "$SWIFT_OUTPUT" single "$C_TRACE" "$C_TRACE"; then
    echo 'phase85dg single-artifact fence unexpectedly accepted' >&2
    exit 1
fi
...
if "$SWIFT_OUTPUT" audit "$C_TRACE" "$SWIFT_TRACE" "$ASAN_TRACE" \
    ...
    audit_status=$?
    echo 'phase85dg fail-closed: next full-trace mismatch remains unmapped' >&2
fi

git -c core.fsmonitor=false diff --check -- \
    src/pc/sm64_modern_gameplay_parity.c \
    script/test_phase85dg_hard_air_mapping.sh \
    tests/sm64_modern_phase85dg_hard_air_mapping.swift \
    .porting/porting-handoff-full-swift-twin-phase85dg-hard-air-mapping.md

printf '%s\n' \
    'SM64 Modern Phase 85dg hard-air mapping matrix completed' \
    'native_c_lifecycle=1 castle_area2=1 sequence12_tick=63 records=476365' \
    'subject37_source=bhvHardAirKnockBackWarp yaw180=1 semantic_identity=0x53f6c1e071460d11' \
