...
M30i transports every resident bank into a canonical little-endian `MFPB` v1
...
8. **M30 Goddard/Mario face.** M30a captures the pointer-free Goddard
   ...
   immutable frame/adjacent-frame lookup. Next drive every expression channel
   from resident payloads (including empty-bank/fallback behavior), and
   compare resource IDs, decoded transforms, and route-specific records before
   resolving face mesh/material/lighting resources through the content pack,
   reproducing camera orientation and all cutscene overrides, and wiring the
