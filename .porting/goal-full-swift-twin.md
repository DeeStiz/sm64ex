...
M30o is the latest validated bounded Goddard/Mario-face route-shard oracle slice. Swift now
...
8. **M30 Goddard/Mario face.** M30a captures the pointer-free Goddard
   ...
   storage, residency, sampler, and encoder-domain policy. Next drive those
   route IDs from real front-end, gameplay, cutscene, and ending entry points,
   compare texture/material/camera records and route ordering. M30q now
   provides validated RGBA8 staging values for the 19 source assets. Next
   attach one real route's staging values to the owner-thread upload path,
   validate private texture residency and upload/fragment barriers, then
   implement matched meshes/materials in a Metal 4 face encoder before
   considering a face-renderer authority cutover.
