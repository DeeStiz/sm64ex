...
In progress — M0, M1, and M2 are complete for their local code/test scopes; M3 now has schema-4 codecs, a live C tick-boundary bridge, a deterministic source inventory, and native hooks for save bytes, RNG, collision, script/behavior, audio sequencing, and render packets. M4 has started with strict Swift 6 deterministic scalar, fixed-point, RNG, paired-timebase, and animation-clock counterparts. File-backed GUI capture, full-save boot/recovery qualification, whole-inventory execution closure, and the remaining M4 trig/animation differential work remain. GUI, physical, distribution, and human gates remain explicitly tracked as external acceptance.
...
| M4: Deterministic primitives | Swift math, fixed-point, trig, RNG, cadence, animation, and timebase operations match C exactly. | In progress — strict Swift 6 scalar/float-bit helpers, fixed 16.16 arithmetic, C-matching RNG vectors, paired timebase model, and integer/fixed animation clock smoke pass; canonical trig/arctan tables and full animation VM differential coverage remain |
