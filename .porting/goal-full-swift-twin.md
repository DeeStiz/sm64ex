...
M23g is the latest validated persistence slice layered on M34a/M33f/M22bc;
the C progression event ABI now carries exact save mutation operands, and the
Swift migration service seeds the normalized image from canonical C snapshots,
replays payloads through the normalized adapter, and fail-closes on byte
divergence. M23f's normalized mutation boundary, M23e's owner-thread
admission/dirty boundary, M23d's value kernel, and M23c's copy/erase
operations remain in force, with independent Swift/C replay fingerprints. M22
remains behavior-coverage work in progress: the
...
| M23: Save system | Swift save parsing, mutation, checksums, atomic persistence, recovery, and bidirectional C compatibility pass. | In progress — M23a–M23g cover repair/upgrade, queries, owner-thread copy/erase, pure mutators, runtime dirty/admission integration, normalized-image replay, and mutation-event payloads; options, restart-selector parity, persistent bidirectional replay, and complete C↔Swift authority closure remain |
