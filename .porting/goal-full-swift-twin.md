...
| M5: Engine state and identity | Swift pools, arenas, stable IDs, references, reset behavior, and object initialization match C. | In progress — owner-thread-only fixed-capacity object pool, C list/update order, free-list allocation/reuse, unimportant eviction, generation-fenced IDs, stale-reference rejection, reset, initialization defaults, and Swift/C contract smoke pass; arenas and full engine-state differential coverage remain |
