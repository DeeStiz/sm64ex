...
M30s is the latest validated bounded Goddard/Mario-face private-residency and barrier slice. Swift now
...
8. **M30 Goddard/Mario face.** M30a captures the pointer-free Goddard
   ...
   Next import the source dynlist vertex/index/material bytes, create the face
   mesh/material argument packet, and bind one actual face draw in a separate
   Metal 4 encoder while retaining the existing C draw as the comparison
   authority. Route IDs still need real front-end, gameplay, cutscene, and
   ending entry-point invocation and complete texture/material/camera ordering
   evidence.
