...
M30v is the latest validated bounded Goddard/Mario-face source-geometry and
...
| M30w: Mario-face texture semantics and complete C draw-list comparison | `SM64MarioFaceMetalDrawListPacket` expands every 877 source triangles into schema-4 command records with material IDs, source indices, texture ID/sampler policy, and transform fingerprint; the dedicated `0x1000_0700` Metal 4 pipeline binds the private IA8→RGBA8 face-shine texture, generated source-normal coordinates, and authored material colors. | Complete locally — `script/test_mario_face_draw_list.sh` matches 880 C↔Swift records, texture fingerprint `0x5958ebfdbac9db39`, material-index fingerprint `0x76db9022f518b224`, source-index fingerprint `0xe52ad05f9a25f2b0`, draw-list fingerprint `0x09bfea1fd886d95f`, and trace fingerprint `0x8cb00ff0996197e6`, with command-17 tamper first divergence `19`; regenerated native Debug build `/tmp/sm64-modern-m30w-build-1.log` and gated `SM64_MODERN_MARIO_FACE_TEXTURE_UPLOAD=1 SM64_MODERN_MARIO_FACE_DRAW=1 SM64_MODERN_MARIO_FACE_TEXTURE_DRAW=1 script/build_and_run.sh --verify` (`/tmp/sm64-modern-m30w-full-verify.log`, `verify_exit=0`) pass private texture residency, `mario_face_texture_draw ... texture_id=768 sampler=1`, full transform/material draw, Apple M5 Max Metal 4 frame one, clean Metal drain, status-0 engine shutdown, and application stop; exact Goddard normal generation, dynamic frame selection, route authority, GPU/screenshot, and visual/human acceptance remain |
