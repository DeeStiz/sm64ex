...
M30r is the latest validated bounded Goddard/Mario-face owner-thread admission slice. Swift now
...
8. **M30 Goddard/Mario face.** M30a captures the pointer-free Goddard
   ...
   validated RGBA8 staging values for the 19 source assets, and M30r admits
   one Mario-normal route through the real owner-thread create/sampler/upload
   path with an explicit pending-residency receipt. Next bind the display-link
   preparation boundary and compare private texture residency plus upload/
   fragment barriers, then implement matched meshes/materials in a Metal 4
   face encoder before considering a face-renderer authority cutover. Route IDs
