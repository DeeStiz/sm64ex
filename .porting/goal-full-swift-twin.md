...
| M17: Progression actors | Stars, coins, lives, caps, switches, doors, warps, cannons, checkpoints, and secrets match C. | In progress — M17a value-only progression schemas/reducer pass strict Swift/C fingerprints; actor behaviors, checksum persistence, dialog/camera/audio delivery, and level-specific reward routes remain |
...
1. **M17 progression actors.** Start with the M17a value reducer for stars,
   ...
   and secrets. Then define stable Swift actor schemas and event reducers for
   red coins and level-completion rewards; port spawn/despawn, collection
   persistence, interaction priority, animation/sound/rumble/render intents,
   and checksum-backed save mutations. Differential routes must cover fresh
   save, repeat collection, death/reload, warp, cap loss, and multiplayer/demo
   inputs where the C build exposes them.
