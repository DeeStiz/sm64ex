...
| M17: Progression actors | Stars, coins, lives, caps, switches, doors, warps, cannons, checkpoints, and secrets match C. | In progress — M17a progression schemas/reducer and M17b C-compatible SaveFile codec/recovery pass strict Swift/C fingerprints; actor behaviors, EEPROM/menu persistence, dialog/camera/audio delivery, and level-specific reward routes remain |
...
1. **M17 progression actors.** Start with the M17a value reducer for stars,
   ...
   and secrets, and the M17b C-compatible SaveFile snapshot/recovery codec.
   Then define stable Swift actor schemas and event reducers for red coins and
   level-completion rewards; port spawn/despawn, collection persistence,
   interaction priority, animation/sound/rumble/render intents, atomic writes,
   and menu-data synchronization. Differential routes must cover fresh save,
   repeat collection, checksum corruption/recovery, death/reload, warp, cap
   loss, and multiplayer/demo inputs where the C build exposes them.
