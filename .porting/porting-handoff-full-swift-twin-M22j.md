# SM64 Modern Full Swift Twin — M22j Handoff

## Scope

M22j registers the existing `bhvAmp` owner bridge in
`SM64BehaviorDispatchBridge`. The shared owner-thread scheduler now dispatches
the general-actor Amp family by exact behavior identity, preserves the
general-actor-before-default-list traversal order, synchronizes the
generation-safe object record, and returns Amp effect records with the
pendulum and respawner receipts. Unknown identities remain explicit
`unmigrated` events; no C-shaped fallback is implied.

This closes a third live owner route only. It does not close the remaining
reachable adapters, real audio/render/collision consumers, progression/save
authority, or whole-game parity.

## Evidence

- `script/test_behavior_dispatch_bridge.sh` — strict Swift/C three-route
  fingerprint `0xa464ccdf2c50b906`; fixed Amp effect ordering, pendulum and
  respawner dispatch, cross-list scheduler order, child allocation, transfer
  fields, and deletion delivery match.
...
- `/tmp/sm64-modern-m22j-build.log` — regenerated native Debug build,
  `BUILD SUCCEEDED`.
- `/tmp/sm64-modern-m22j-final-matrix.log` — complete matrix,
  `runs=206 failures=0` (bounded chunk logs are
  `/tmp/sm64-modern-m22j-matrix-01.log` through `-09.log`).
