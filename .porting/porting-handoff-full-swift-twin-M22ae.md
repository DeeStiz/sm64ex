# SM64 Modern Full Swift Twin — M22ae Handoff

## Scope

M22ae registers the existing Bob-omb Buddy owner bridge and its exact buddy
and cannon-closed behavior identities in the shared
`SM64BehaviorDispatchBridge`. The general-actor route now advances the value
reducer in the shared scheduler, preserves generation-safe state and record
publication, and returns explicit owner-thread sound/dialog/camera/time-stop
delivery receipts. Unknown identities remain fail-closed as `unmigrated`
events.

This closes a twenty-fifth behavior identity family route only. The focused
shared fixture covers the idle owner record, exact identity dispatch, and an
explicit empty owner delivery. The standalone differential remains the
authority for conversation, cannon preparation, time-stop, and interaction
cleanup branches; real dialog/camera/audio consumers, collision admission,
progression/save, remaining adapters, and whole-game parity remain open. The
behavior inventory remains 534 rows with 73 known Swift value/owner identities
and 461 explicit `unmigrated_c_adapter` rows.

## Evidence

- `script/test_behavior_dispatch_bridge.sh` — strict Swift/C twenty-five-route
  fingerprint `0x9e95baecbd610899`; Bob-omb Buddy dispatches in the shared
  general-actor traversal.
- `script/test_bobomb_buddy_object_bridge.sh` — standalone Swift/C Bob-omb
  Buddy fingerprint `0xba317f5f6079097e`; reducer and owner bridge remain
  matched.
- `script/test_engine_runtime.sh`, `script/test_live_route_oracle.sh`, and
  `script/test_live_route_promotion.sh` — Swift 6 engine-context and schema-4
  live-route contracts pass with the expanded dispatcher dependencies.
- `script/test_behavior_manifest.sh` — fingerprint
  `0x9bb2863444c51d6d`; 534 rows, 73 `swift_value_owner` routes, and 461
  `unmigrated_c_adapter` rows.
- `/tmp/sm64-modern-m22ae-build.log` — regenerated native Swift 6 Debug build,
  `BUILD SUCCEEDED`.
- `/tmp/sm64-modern-m22ae-final-matrix.log` — all 206 script indices replayed
  with each script's declared interpreter, `runs=206 failures=0`.
- `git diff --check` — clean; strict-concurrency audit reports zero
  `@unchecked Sendable` declarations in the audited runtime.

## Remaining work

1. Register every remaining already-proven Swift owner bridge and attach level
   spawns through exact behavior identity, including composite parents whose
   child bridges still own separate scheduler shadows.
2. Migrate the remaining 461 reachable adapters/transient children, or record
   an explicitly approved compatibility exception for each one.
3. Complete Bob-omb Buddy's real dialog/camera/audio consumers, collision,
   progression/save, and front-end integration before claiming full parity.
4. Continue M23–M35: save/configuration/HUD/front-end, audio, display-list
   translation, Goddard, whole-engine authority, strict parity-shard
   qualification, Metal 4 production validation, and release/human gates.

## Next command

Inspect the next unregistered owner bridge, add its external-tick boundary and
exact behavior identities, then repeat the focused Swift/C, runtime/oracle,
regenerated native build, shebang-aware matrix, strict audit, and handoff
checks before committing.
