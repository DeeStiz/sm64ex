# SM64 Modern Full Swift Twin — M33jf Handoff

## Scope

M33jf closes the parent-presentation `bhvCannonBarrel` route. The typed owner
copies the cannon parent position, yaw, and pitch, preserves default-list
child ordering, and hides the barrel when the parent is inactive.

## Evidence

- Dispatch/parent-copy smoke, manifest, Metal 4 source, engine runtime,
  live-route oracle, route-shard replay, timebase, shell syntax, and
  `git diff --check` pass.
- Manifest: 534 rows, 417 Swift-owned, 117 C adapters,
  fingerprint `0xe2a1348113f12828`.
