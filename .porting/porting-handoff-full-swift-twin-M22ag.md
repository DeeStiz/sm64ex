# SM64 Modern Full Swift Twin — M22ag Handoff

## Scope

M22ag registers the existing Bouncing Fireball parent and flame owner bridges
in the shared `SM64BehaviorDispatchBridge`. The general-actor flame route runs
before the default-list parent route in the shared scheduler, preserving the C
cross-list order, value effects, child identity, and generation-safe records.
Unknown identities remain fail-closed as `unmigrated` events.

This closes the parent/flame behavior identity pair only. The focused shared
fixture covers independent flame/parent dispatch and cross-list ordering; the
standalone differential remains the authority for activation, flame spawning,
surface deletion, and long-lived bounce timing. Full collision and presentation
...
- `script/test_behavior_dispatch_bridge.sh` — strict Swift/C twenty-eight-route
  fingerprint `0xeba65f4903d8f9f4`; flame and parent dispatch in the shared
  cross-list traversal.
- `script/test_bouncing_fireball.sh` — standalone Swift/C Bouncing Fireball
  fingerprint `0x6ad7b0bf4304989e`; parent/flame reducer and owner bridge match.
...
- `/tmp/sm64-modern-m22ag-build.log` — regenerated native Swift 6 Debug build,
  `BUILD SUCCEEDED`.
- `/tmp/sm64-modern-m22ag-final-matrix.log` — all 206 script indices replayed
...
3. Complete Bouncing Fireball collision/presentation integration and its real
   gameplay consumers before claiming full behavior parity.
