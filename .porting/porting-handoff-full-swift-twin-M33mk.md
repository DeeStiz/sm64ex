# SM64 Modern Full Swift Twin — M33mk Handoff

## Scope

M33mk adds `bhvKoopaRaceEndpoint` as a Swift parent owner. It preserves the
race-begun/ended gate, Koopa-finished and 400-unit distance admission,
timer-stop/fanfare intent, cannon-shot `-1` status, normal-win `1` status, and
generation-safe allocation/retirement of its Koopa Flag child.

## Evidence

- Focused Swift/C Koopa Race Endpoint contract matches at
  `0xa651adf251518d63`.
- Aggregate dispatch proves endpoint routing, parent-child allocation,
  completion status, and fanfare; behavior manifest, Metal 4 source contract,
  engine runtime, live-route C replay, coverage, 7,419-shard inventory,
  regenerated strict Swift 6 Xcode Debug build, shell syntax, and
  `git diff --check` pass.
- The full verifier completed with `verify_exit=0` in
  `/tmp/sm64-modern-m33mk-full-verify.log`: the complete matrix/build passed,
...
presentation, and the log records `engine_thread_finished status=0` plus
  `application_stopped`; the same host log records `audio_service_stopped ...
  underrun=75`, so sustained audio/device quality remains open.
- Manifest: 534 rows, 453 Swift-owned, 81 explicit C adapters,
  fingerprint `0x12bc237011ca07f5`.
