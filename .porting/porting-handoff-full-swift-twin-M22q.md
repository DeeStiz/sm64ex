# SM64 Modern Full Swift Twin — M22q Handoff
...
Bob-omb, Bird, Swoop, Piranha Plant, Big Boo, and Fly Guy in general-actor
list order, preserves Fly Guy's idle/oscillation state and generation-safe
flame compatibility seam, and leaves the Bob-omb smoke child and respawner
...
This closes a tenth live owner route only. It does not close the remaining
...
- `script/test_behavior_dispatch_bridge.sh` — strict Swift/C ten-route
  fingerprint `0xc587e7525da91eef`; Amp/Boo/Bob-omb/Bird/Swoop/Piranha
  Plant/Big Boo/Fly Guy general-actor order, default-list pendulum/respawner
  traversal, compatibility-child identities, and owner delivery match.
- `script/test_fly_guy_object_bridge.sh` — existing standalone Fly Guy
  value/owner differential remains `0x2fbf98eca64a5496`.
...
- `/tmp/sm64-modern-m22q-build.log` — regenerated native Debug build,
  `BUILD SUCCEEDED`.
- `/tmp/sm64-modern-m22q-final-matrix.log` — complete matrix,
  `runs=206 failures=0` (bounded chunk logs are
  `/tmp/sm64-modern-m22q-matrix-01.log` through `-09.log`).
