# SM64 Modern Full Swift Twin — M33mt Handoff

## Scope

M33mt adds `bhvSLSnowmanWind` as a Swift environment/NPC owner. It preserves
idle textbox admission, dialog completion, ±`0x1500` yaw clamp, 12 strong-wind
particle intent, dialog ID 153, and the wind-sound edge.

## Evidence

- Focused Swift/C Snowman Wind contract matches at
  `0x0bb6ba5b32436749`.
- Aggregate dispatch proves dialog and yaw-clamp routing; behavior manifest,
...
- The full verifier completed with `verify_exit=0` in
  `/tmp/sm64-modern-m33mt-full-verify.log`: the complete matrix/build passed,
...
- Manifest: 534 rows, 464 Swift-owned, 70 explicit C adapters,
  fingerprint `0x5395bd13d5a9f195`.
