# SM64 Modern Full Swift Twin — M22ax Handoff

## Scope

M22ax registers Big Boo's bridge-owned static staircase identity in the shared
`SM64BehaviorDispatchBridge`. The identity selects the Big Boo parent owner
route, whose standalone bridge allocates the three level-list staircase child
records and applies their source transforms. No second child scheduler is
introduced.

This closes staircase identity selection only. Staircase collision,
presentation, and whole-game parity remain open. The behavior inventory remains
534 rows with 73 known Swift value/owner identities and 461 explicit
`unmigrated_c_adapter` rows.

## Evidence

- `script/test_behavior_dispatch_bridge.sh` — strict Swift/C forty-two-route
  identity fingerprint `0x6e434328de50d0f9`; the staircase identity shares the
  Big Boo owner route.
- `script/test_big_boo_object_bridge.sh` — standalone Big Boo owner contract
  covers parent retirement, three staircase children, source transforms, and
  reward/presentation seams.
...
- `script/test_behavior_manifest.sh` — fingerprint
  ...
  `unmigrated_c_adapter` rows.
- `/tmp/sm64-modern-m22ax-build.log` — regenerated native Swift 6 Debug build,
  `BUILD SUCCEEDED`.
- `/tmp/sm64-modern-m22ax-final-matrix.log` — all 206 script indices replayed
...
3. Complete Big Boo staircase collision/presentation consumers and whole-game
   behavior parity before claiming the route fully integrated.
