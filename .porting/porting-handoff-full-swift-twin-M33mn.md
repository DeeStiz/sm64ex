# SM64 Modern Full Swift Twin — M33mn Handoff

## Scope

M33mn adds `bhvYellowBackgroundInMenu` as a Swift menu-surface owner. It
preserves the authored `0x8000` yaw initializer, constant 9.0 scale, per-tick
transform publication, and level-menu ownership.

## Evidence

- Focused Swift/C Yellow Background Menu contract matches at
  `0x52a54fdc3009b604`.
- Aggregate dispatch proves menu-surface routing and scale/rotation output;
...
- The full verifier completed with `verify_exit=0` in
  `/tmp/sm64-modern-m33mn-full-verify.log`: the complete matrix/build passed,
...
- Manifest: 534 rows, 457 Swift-owned, 77 explicit C adapters,
  fingerprint `0x577544db49bf44a3`.
