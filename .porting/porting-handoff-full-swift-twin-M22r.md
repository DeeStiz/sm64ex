# SM64 Modern Full Swift Twin — M22r Handoff
...
general-actor list order, preserves Bullet Bill reset/waiting state and the
explicit smoke-delivery seam, and leaves the Bob-omb smoke child and respawner
replacement child as explicit `unmigrated` events. Unknown identities remain
fail-closed; no route silently receives a C-shaped fallback.

This closes an eleventh live owner route only. It does not close the remaining
reachable adapters, transient-child breadth, real audio/render/collision
consumers, progression/save authority, or whole-game parity.

## Evidence

- `script/test_behavior_dispatch_bridge.sh` — strict Swift/C eleven-route
  fingerprint `0x06525082a1b0853c`; Amp/Boo/Bob-omb/Bird/Swoop/Piranha Plant/
  Big Boo/Fly Guy/Bullet Bill general-actor order, default-list
  pendulum/respawner traversal, compatibility-child identities, and owner
  delivery match.
- `script/test_bullet_bill_object_bridge.sh` — existing standalone Bullet Bill
  value/owner differential remains `0x98814f6acf3e0025`.
...
- `/tmp/sm64-modern-m22r-build.log` — regenerated native Debug build,
  `BUILD SUCCEEDED`.
- `/tmp/sm64-modern-m22r-final-matrix.log` — complete matrix,
  `runs=206 failures=0` (bounded chunk logs are
  `/tmp/sm64-modern-m22r-matrix-01.log` through `-09.log`).
