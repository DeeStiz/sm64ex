# SM64 Modern Full Swift Twin — M33gw Handoff

## Scope

M33gw migrates `bhvAlphaBooKey` and `bhvBetaBooKey` through one Swift 6
parent/drop interaction owner. It preserves alpha/beta rotation, parent-death
transition, 40-unit drop velocity, dropped-state roll settling, the 90-frame
tangibility fence, parent handoff, collision collection, and golden-sparkle
ownership.

## Evidence

- Focused Swift/C Boo-key fingerprint:
  `0xde6afa5beafcc025` from `script/test_boo_key.sh`.
- Dispatch smoke fingerprint: `0x681ceb2358bf2e21`; it exercises both key
  identities through the production bridge.
- Behavior manifest: `0x878503ce8420d210`, 534 rows, 325 Swift value/owner
  routes, and 209 explicit C adapters.
...
- Regenerated strict Swift 6/macOS 27 Debug build passes in
  `build/sm64-modern-m33gw-xcode` with `** BUILD SUCCEEDED **`.
