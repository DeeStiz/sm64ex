# SM64 Modern Full Swift Twin — M33mq Handoff

## Scope

M33mq adds `bhvSpindrift` as a Swift enemy owner. It preserves hitbox/loot
values, forward-velocity approach, home/Mario yaw admission, attack-to-recoil
transition, 20-frame recovery/reset, and movement ownership.

## Evidence

- Focused Swift/C Spindrift contract matches at
  `0x9ac8294303fff174`.
- Aggregate dispatch proves attack/death, movement, and recovery; behavior
  manifest, Metal 4 source contract, engine runtime, live-route C replay,
  coverage, 7,419-shard inventory, regenerated strict Swift 6 Xcode Debug
  build, shell syntax, and `git diff --check` pass.
- The full verifier completed with `verify_exit=0` in
  `/tmp/sm64-modern-m33mq-full-verify.log`: the complete matrix/build passed,
...
- Manifest: 534 rows, 461 Swift-owned, 73 explicit C adapters,
  fingerprint `0x3c40bb596263a099`.
