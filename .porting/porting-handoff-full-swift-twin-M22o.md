# SM64 Modern Full Swift Twin — M22o Handoff
...
Bob-omb, Bird, Swoop, and Piranha Plant in general-actor list order, preserves
the plant's idle/sleeping/tangible state and generation-safe records, and
leaves the Bob-omb smoke child and respawner replacement child as explicit
`unmigrated` events. Unknown identities remain fail-closed; no route silently
receives a C-shaped fallback.

This closes an eighth live owner route only. It does not close the remaining
...
- `script/test_behavior_dispatch_bridge.sh` — strict Swift/C eight-route
  fingerprint `0x3232102d6c9b1483`; Amp/Boo/Bob-omb/Bird/Swoop/Piranha Plant
  general-actor order, default-list pendulum/respawner traversal,
  compatibility-child identities, and owner delivery match.
...
- `script/test_piranha_plant_object_bridge.sh` — existing standalone Piranha
  Plant value/owner differential remains `0x4202eefc24547aa0`.
...
- `/tmp/sm64-modern-m22o-build.log` — regenerated native Debug build,
  `BUILD SUCCEEDED`.
- `/tmp/sm64-modern-m22o-final-matrix.log` — complete matrix,
  `runs=206 failures=0` (bounded chunk logs are
  `/tmp/sm64-modern-m22o-matrix-01.log` through `-09.log`).
