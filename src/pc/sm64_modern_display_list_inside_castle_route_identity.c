...
/*
 * The observer is deliberately narrow: only the authored area-2/3 parent
 * reaches this branch, and only its authored leaf is copied.  The packet is
 * fixed-width and pointer-free before the route records any values.
 */
static SM64ModernDisplayListInsideCastleRoutePacketV1 sLastPacket;
...
static bool target_parent_is_authored(const void *display_list) {
    return display_list == (const void *) inside_castle_seg7_dl_07043B48;
}

static bool parent_references_target_leaf(const Gfx *parent) {
    const Gfx *target =
        sm64_modern_inside_castle_display_list_07043A68_source();
    ...
    for (uint32_t index = 0; index < 64u; ++index) {
        ...
        if (opcode == G_ENDDL) return false;
        if (opcode == G_DL && word1 == target_address) return true;
    }
    return false;
}

static SM64ModernStatus copy_target_leaf(
    SM64ModernDisplayListInsideCastleRoutePacketV1 *packet,
    uint32_t drawing_layer) {
    const Gfx *leaf =
        sm64_modern_inside_castle_display_list_07043A68_source();
    uint32_t normalized_words[
        SM64_MODERN_DISPLAY_LIST_INSIDE_CASTLE_ROUTE_WORD_CAPACITY * 2u] = {0};
    uint32_t resource_ids[
        SM64_MODERN_DISPLAY_LIST_INSIDE_CASTLE_ROUTE_WORD_CAPACITY] = {0};
    uint32_t triangle_count = 0;

    for (uint32_t index = 0;
         index < SM64_MODERN_DISPLAY_LIST_INSIDE_CASTLE_ROUTE_WORD_CAPACITY;
         ++index) {
        ...
        switch (index) {
            case 0u:
                ...
                normalized_word1 =
                    SM64_MODERN_DISPLAY_LIST_INSIDE_CASTLE_ROUTE_TEXTURE_RESOURCE_ID;
            ...
            case 3u:
                ...
                normalized_word1 =
                    SM64_MODERN_DISPLAY_LIST_INSIDE_CASTLE_ROUTE_VERTEX_RESOURCE_ID;
                resource_id = normalized_word1;
                break;
            case 4u:
    ...
    if (triangle_count != 2u) return SM64_MODERN_STATUS_PARITY_DIVERGED;

    memset(packet, 0, sizeof(*packet));
    packet->header.abi_version = SM64_MODERN_ABI_VERSION_1;
    packet->header.struct_size = sizeof(*packet);
    packet->source_identity =
        SM64_MODERN_DISPLAY_LIST_INSIDE_CASTLE_ROUTE_SOURCE_ID;
    packet->owner_identity =
        SM64_MODERN_DISPLAY_LIST_INSIDE_CASTLE_ROUTE_OWNER_ID;
    packet->word_count = SM64_MODERN_DISPLAY_LIST_INSIDE_CASTLE_ROUTE_WORD_CAPACITY;
    packet->drawing_layer = drawing_layer;
    packet->triangle_count = triangle_count;
    packet->flags = SM64_MODERN_DISPLAY_LIST_INSIDE_CASTLE_ROUTE_FLAG_SCENE_GRAPH_OWNER
        | SM64_MODERN_DISPLAY_LIST_INSIDE_CASTLE_ROUTE_FLAG_POINTERS_NORMALIZED;
...
SM64ModernStatus sm64_modern_display_list_inside_castle_route_observe_scene_graph_append(
    ...
    uint32_t drawing_layer) {
    sInvocations++;
    if (!target_parent_is_authored(display_list)) return SM64_MODERN_STATUS_OK;
    ...
    return sm64_modern_oracle_trace_record(
        ...
        SM64_MODERN_DISPLAY_LIST_INSIDE_CASTLE_ROUTE_EVENT,
        sLastPacket.flags,
        values,
        8u);
}
