...
/*
 * This observer is intentionally narrow.  The scene graph passes the parent
 * list to geo_append_display_list(), while the selected leaf is reached by a
 * real G_DL command inside that parent.  We inspect only the authored parent
 * and copy the authored leaf immediately; no generic pointer traversal is
 * exposed to the Swift side.
 */
static SM64ModernDisplayListRoutePacketV1 sLastPacket;
...
static bool target_parent_is_authored(const void *display_list) {
    /* wooden_door_geo -> door_seg3_dl_03014A80 is the selected actor seam. */
    return display_list == (const void *) door_seg3_dl_03014A80;
}

static bool parent_references_target_leaf(const Gfx *parent) {
    const uint32_t target_address = (uint32_t) (uintptr_t) door_seg3_dl_03014A20;
...
static SM64ModernStatus copy_target_leaf(
    SM64ModernDisplayListRoutePacketV1 *packet,
    uint32_t drawing_layer) {
    const Gfx *leaf = door_seg3_dl_03014A20;
    uint32_t normalized_words[SM64_MODERN_DISPLAY_LIST_ROUTE_WORD_CAPACITY * 2u] = {0};
    uint32_t resource_ids[SM64_MODERN_DISPLAY_LIST_ROUTE_WORD_CAPACITY] = {0};
    uint32_t word_count = 0;
    uint32_t triangle_count = 0;
    bool saw_end = false;

    for (uint32_t index = 0; index < SM64_MODERN_DISPLAY_LIST_ROUTE_WORD_CAPACITY; ++index) {
        ...
        if (opcode == G_VTX) {
            /* The source segment address is stable; the host pointer is not. */
            if (index != 0u) {
                return SM64_MODERN_STATUS_PARITY_DIVERGED;
            }
            normalized_word1 = SM64_MODERN_DISPLAY_LIST_ROUTE_VERTEX_RESOURCE_ID;
            resource_id = SM64_MODERN_DISPLAY_LIST_ROUTE_VERTEX_RESOURCE_ID;
    ...
    if (!saw_end || word_count != 4u || triangle_count != 4u) {
        return SM64_MODERN_STATUS_PARITY_DIVERGED;
    }
    ...
    packet->source_identity = SM64_MODERN_DISPLAY_LIST_ROUTE_SOURCE_ID;
    packet->owner_identity = SM64_MODERN_DISPLAY_LIST_ROUTE_OWNER_ID;
    packet->word_count = word_count;
    packet->drawing_layer = drawing_layer;
    packet->triangle_count = triangle_count;
    packet->flags = SM64_MODERN_DISPLAY_LIST_ROUTE_FLAG_SCENE_GRAPH_OWNER
        | SM64_MODERN_DISPLAY_LIST_ROUTE_FLAG_POINTERS_NORMALIZED;
...
SM64ModernStatus sm64_modern_display_list_route_observe_scene_graph_append(
    ...
    return sm64_modern_oracle_trace_record(
        ...
        SM64_MODERN_DISPLAY_LIST_ROUTE_EVENT,
        sLastPacket.flags,
        values,
        8u);
}
